...
def build_episode_chain(
    predecessor: dict, successor: dict, *, max_gap_minutes: int = 360
) -> EpisodeRecord:
    for key in (
        "flight_id",
        "aircraft_id",
    ...
    if (predecessor["aircraft_id"], predecessor["aircraft_id_namespace"]) != (
        successor["aircraft_id"],
        successor["aircraft_id_namespace"],
    ):
    ...
    gap = (
        successor["event_start_time"] - predecessor["event_end_time"]
    ).total_seconds() / 60
    ...
    payload = {
        ...
        "rule": f"{CHAIN_RULE_ID}_{max_gap_minutes}",
    }
    ...
    return EpisodeRecord(
        ...
        episode_start_time=predecessor["event_start_time"],
        episode_end_time=successor["event_end_time"],
        ...
        join_keys=CHAIN_JOIN_KEYS,
        ordering_rule=CHAIN_ORDERING_RULE,
        continuity_rule=CHAIN_CONTINUITY_RULE,
        source_record_ids=source_record_ids,
        construction_provenance=source_record_ids
        + (f"{CHAIN_RULE_ID}@{CHAIN_RULE_VERSION}",),
...
def build_episode_records(
    flights: list[dict], *, max_gap_minutes: int = 360
) -> list[EpisodeRecord]:
    required = (
        "dataset_instance_id",
        "aircraft_id_namespace",
...
def build_data2_episode_chain(
    ...
    payload = {
        ...
        "predecessor": predecessor["flight_id"],
        "successor": successor["flight_id"],
        "rule": f"{DATA2_CHAIN_RULE_ID}_{max_gap_minutes}",
    }
    source_record_ids = (_source_record_id(predecessor), _source_record_id(successor))
    return EpisodeRecord(
        episode_id=content_id(payload),
        ...
        chain_rule_parameters=(
            f"max_gap_minutes={max_gap_minutes}",
            "gap_source=actual_gate_utc",
        ...
        source_record_ids=source_record_ids,
        construction_provenance=source_record_ids
        + (f"{DATA2_CHAIN_RULE_ID}@{DATA2_CHAIN_RULE_VERSION}",),
        lineage_support=SupportState.SUPPORTED,
        formal_eligible=True,
        quality_flags=(),
    )
...
def build_data2_episode_records(
    flights: list[dict], *, max_gap_minutes: int = 360
) -> list[EpisodeRecord]:
    required = (
        "dataset_instance_id",
