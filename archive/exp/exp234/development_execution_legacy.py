"""Exp2-Exp4 Development execution on the current typed M1 V2 scenarios.
...
from archive.exp.exp234.exp234_helpers_legacy import (
    FLIGHT_SCOPE,
    FULL_SCOPE,
    _ACTION_META,
    _action_family,
...
CURRENT_SCENARIO_ARTIFACT = ROOT / "artifacts" / "experiment" / "m1_v2_current_stage_scenarios_v4" / "M1_V2_CURRENT_STAGE_TYPED_JOINT_SCENARIOS.json"
CURRENT_SCENARIO_MANIFEST = ROOT / "artifacts" / "experiment" / "m1_v2_current_stage_scenarios_v4" / "M1_V2_CURRENT_STAGE_TYPED_JOINT_SCENARIO_MANIFEST.json"
CURRENT_PRE_INPUTS = ROOT / "artifacts" / "diagnostics" / "m1_v2_development_current_stage_refreeze_v3" / "M1_V2_CURRENT_STAGE_DEVELOPMENT_INFERENCE_INPUTS.json"
# Kept only for backwards-compatible output locations.  The source is now the
# current-stage typed V2 artifact above, never the legacy signed V1 parquet.
...
def load_scenario_artifact() -> tuple[list[dict], dict[str, list[dict]], dict]:
    """Load the current-stage V2 typed rows and bind PRE airport identity.

    The legacy V1 parquet uses ``delta_ob``/``t_tx``.  Those names are not
    accepted here.  V2 rows are converted only from their strict
    ``D_OB``/``D_TX`` fields and retain typed tail/abstain support.
    """
    manifest = _read_json(CURRENT_SCENARIO_MANIFEST)
    payload = _read_json(CURRENT_SCENARIO_ARTIFACT)
    if manifest.get("status") != "M1_CURRENT_STAGE_JOINT_SCENARIO_ARTIFACT_MATERIALIZED":
        raise RuntimeError("EXP234_CURRENT_V2_SCENARIO_STATUS_INVALID")
    if manifest.get("artifact_hash") != payload.get("artifact_hash"):
        raise RuntimeError("EXP234_CURRENT_V2_SCENARIO_HASH_MISMATCH")
    safety = manifest.get("safety", {})
    if safety.get("FINAL_TEST_ACCESS_COUNT") != 0 or safety.get("PAPER_FULL_RUN") is not False:
        raise RuntimeError("EXP234_SCENARIO_ARTIFACT_FINAL_TEST_VIOLATION")
    pre_inputs = _read_json(CURRENT_PRE_INPUTS)
    pre_by_node = {}
    for states in pre_inputs["pre_states_by_episode"].values():
        for state in states:
            ...
            if not isinstance(schedule, dict):
                raise RuntimeError("EXP234_CURRENT_V2_SCHEDULE_REFERENCE_MISSING")
            pre_by_node[node["decision_node_id"]] = {
                "connection_airport_id": str(schedule["origin_airport_id"]),
                "successor_destination_airport_id": str(schedule["destination_airport_id"]),
    ...
    for source in payload["rows"]:
        node_id = source["decision_node_id"]
        binding = pre_by_node.get(node_id)
        if binding is None:
            raise RuntimeError("EXP234_CURRENT_V2_NODE_NOT_BOUND_TO_PRE")
        if node_id not in seen_nodes:
            seen_nodes.add(node_id)
            node_rows.append({
                "episode_id": source["episode_id"],
                "decision_node_id": node_id,
                "operational_stage": source["operational_stage"],
                **binding,
            })
        envelopes = {item["target_name"]: item for item in source["target_envelopes"]}
        def value(target: str):
            item = envelopes[target]
            return item.get("scalar_minutes")
        def support(target: str):
            item = envelopes[target]
            return "SUPPORTED" if item.get("scalar_support_state") == "SUPPORTED" else "ABSTAIN"
        row = {
            "episode_id": source["episode_id"],
            "decision_node_id": node_id,
            "scenario_id": int(source["scenario_id"]),
            "scenario_weight": float(source["scenario_weight"]),
            "r_ib_minutes": value("T_IB_A00"),
            "d_ob_minutes": value("D_OB"),
            "d_tx_minutes": value("D_TX"),
            "d_to_minutes": source.get("D_TO"),
            "r_ib_support": support("T_IB_A00"),
            "d_ob_support": support("D_OB"),
            "d_tx_support": support("D_TX"),
            "d_to_support": "SUPPORTED" if source.get("D_TO") is not None else "ABSTAIN",
            "operational_stage": source["operational_stage"],
            "scenario_seed_key": source["scenario_seed_key"],
            "target_envelopes": source["target_envelopes"],
            "lineage": source["lineage"],
        }
        by_node.setdefault(node_id, []).append(row)
    for rows in by_node.values():
        rows.sort(key=lambda row: int(row["scenario_id"]))
    if len(node_rows) != payload["node_count"] or sum(map(len, by_node.values())) != payload["row_count"]:
        raise RuntimeError("EXP234_CURRENT_V2_SCENARIO_CARDINALITY_INVALID")
    return node_rows, by_node, {**manifest, "artifact_hash": payload["artifact_hash"], "source": "CURRENT_M1_V2_TYPED_JSON"}
def load_m2() -> dict:
