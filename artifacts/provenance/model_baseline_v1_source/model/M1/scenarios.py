...
def _sample_hurdle_quantile(
    ...
    ``TEST_ONLY_LINEAR``.
    """
    value, index, overflow, _, _, _ = _sample_hurdle_quantile_with_metadata(
        zero_logit,
        quantile_logits,
        contract,
        uniform,
...
def _sample_hurdle_quantile_with_metadata(
    zero_logit: torch.Tensor,
    quantile_logits: torch.Tensor,
    contract: HurdleQuantileContract,
    uniform: float,
    *,
    tail_continuation: EmpiricalTailContinuation | None,
) -> tuple[float, int, bool, bool, str | None, str | None]:
    """Sample one hurdle draw and retain positive-tail provenance."""
    ...
    if uniform < zero_probability:
        return 0.0, 0, False, False, None, None
    positive_uniform = (uniform - zero_probability) / max(1.0 - zero_probability, 1e-12)
    tail_used = positive_uniform > contract.q_max + 1e-9
    if tail_used:
        if tail_continuation is None and contract.upper_tail_policy == "TEST_ONLY_LINEAR":
            value = float(
                quantile_value(
            ...
            tail_used = False
        elif tail_continuation is None:
            raise ContractError("M1_POSITIVE_TAIL_CONTINUATION_REQUIRED")
        elif tail_continuation.target != contract.target_name:
            raise ContractError("M1_POSITIVE_TAIL_CONTINUATION_TARGET_MISMATCH")
        else:
            v = (positive_uniform - contract.q_max) / (1.0 - contract.q_max)
            tail_excess = tail_continuation.excess_at(v)
            value = float(quantiles[-1]) + tail_excess
    else:
        value = float(
            quantile_value(
                quantiles.unsqueeze(0),
    ...
    return (
        ...
        overflow,
        tail_used,
        tail_continuation.tail_continuation_id if tail_used else None,
        tail_continuation.tail_reference_hash if tail_used else None,
    )
...
def ancestral_sample_v2(
    ...
    *,
    ...
    tail_continuations: dict[str, EmpiricalTailContinuation] | None = None,
) -> tuple[M1V2Scenario, ...]:
    ...
    tail_continuations = dict(tail_continuations or {})
    if (
    ...
    for scenario_id in range(count):
        ...
        if supports.get("D_OB") == "ABSTAIN":
            ...
            (
                ...
                d_ob_contract,
                uniform,
                tail_continuation=tail_continuations.get("D_OB"),
            )
        ...
        if supports.get("D_TX") == "ABSTAIN":
            ...
            d_tx_bin = d_tx_contract.encode(d_tx_minutes)
            overflow_d_tx = d_tx_contract.tail_state(d_tx_bin) == "OVERFLOW"
            ...
            (
                ...
                d_tx_contract,
                uniform,
                tail_continuation=tail_continuations.get("D_TX"),
            )
        ...
        rows.append(
            M1V2Scenario(
                ...
                taxi_reference_fallback_level=taxi_reference_fallback_level,
                taxi_reference_support_state=taxi_reference_support_state,
                positive_tail_used=positive_tail_d_ob or positive_tail_d_tx,
                positive_tail_targets=tuple(
                    target
                    for target, used in (("D_OB", positive_tail_d_ob), ("D_TX", positive_tail_d_tx))
                    if used
                ),
                tail_continuation_id=(
                    tail_d_ob_id or tail_d_tx_id
                    if (positive_tail_d_ob or positive_tail_d_tx)
                    else None
                ),
                tail_reference_hash=(
                    tail_d_ob_hash or tail_d_tx_hash
                    if (positive_tail_d_ob or positive_tail_d_tx)
                    else None
                ),
            )
