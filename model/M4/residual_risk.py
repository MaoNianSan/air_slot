...
from model.M3.action_response import ResponseSupportClass
    ...
    def coherent_risk_envelope(self):
        ...
        if self.support_state is RiskEvaluationSupport.ABSTAINED:
            if any(value is not None for value in metrics):
                raise ValueError("M4_ABSTAINED_RISK_CANNOT_HAVE_METRICS")
            if self.ranking_authority is not RankingAuthority.NOT_RANKED:
                raise ValueError("M4_ABSTAINED_RISK_CANNOT_BE_RANKED")
        elif any(value is None for value in metrics):
            raise ValueError("M4_EVALUATED_RISK_REQUIRES_ALL_METRICS")
        elif self.coverage_fraction != 1.0:
            raise ValueError("M4_EVALUATED_RISK_REQUIRES_FULL_MAPPING_COVERAGE")
        if (
            self.support_state is RiskEvaluationSupport.SUPPORTED
            and self.ranking_authority is not RankingAuthority.AUTHORITATIVE
        ) or (
            self.support_state is RiskEvaluationSupport.ASSUMPTION_BASED
            and self.ranking_authority is not RankingAuthority.CONDITIONAL
        ):
            raise ValueError("M4_RISK_SUPPORT_RANKING_AUTHORITY_MISMATCH")
        return self
...
class RiskRankingEnvelope(FrozenModel):
    ...
    authoritative_ranking: tuple[RiskRankingEntry, ...]
    conditional_ranking: tuple[RiskRankingEntry, ...]
    abstained_action_ids: tuple[str, ...]
...
def _abstained_scenario_loss(scenario, reason: str) -> ScenarioMonetaryLoss:
    components = tuple(
...
def evaluate_residual_risk(
    ...
    *,
    monetary_mapping: MonetaryMappingRegistry,
    ...
    reasons = []
    complete_mapping = set(monetary_mapping.component_mappings) == set(
        CONSEQUENCE_COMPONENTS
    )
    ...
    if can_map:
        for scenario in envelope.scenario_consequences:
            ...
            total = sum(item.L_k_m for item in components)
            payload = {
                "scenario_id": scenario.scenario_id,
                "scenario_weight": scenario.scenario_weight,
        ...
        scenario_losses = [
            _abstained_scenario_loss(scenario, reason)
            for scenario in envelope.scenario_consequences
        ]
    ...
    if can_map:
        ...
        if (
            envelope.response_support is ResponseSupportClass.SUPPORTED
            ...
            authority = RankingAuthority.AUTHORITATIVE
        else:
            support = RiskEvaluationSupport.ASSUMPTION_BASED
            authority = RankingAuthority.CONDITIONAL
            reasons.append("NON_AUTHORITATIVE_INPUT_OR_POLICY")
    else:
        expected = variance = var = cvar = objective = None
        support = RiskEvaluationSupport.ABSTAINED
        authority = RankingAuthority.NOT_RANKED
    ...
    return RiskEvaluationEnvelope(
        ...
        coverage_components=tuple(monetary_mapping.component_mappings),
        coverage_fraction=len(monetary_mapping.component_mappings)
...
def rank_risk_evaluations(
    ...
    if len(identities) != 1:
        raise ValueError("M4_RANKING_MAPPING_OR_POLICY_MISMATCH")
    ...
    system, mapping_hash, policy_hash = next(iter(identities))
    return RiskRankingEnvelope(
        monetary_system_id=system,
        monetary_mapping_registry_hash=mapping_hash,
        risk_policy_hash=policy_hash,
        authoritative_ranking=ranked(RankingAuthority.AUTHORITATIVE),
        conditional_ranking=ranked(RankingAuthority.CONDITIONAL),
        abstained_action_ids=tuple(
            sorted(
                item.action_id
                for item in evaluations
                if item.ranking_authority is RankingAuthority.NOT_RANKED
            )
        ),
    )
