...
def current_history(states: tuple[PREState, ...] | list[PREState]) -> tuple[PREState, ...]:
    """Return only current node while preserving its complete information-state row."""
...
def fixed_history(states: tuple[PREState, ...] | list[PREState],
                  window_minutes: int) -> tuple[PREState, ...]:
    """Return the closed causal interval [t-W, t] on the canonical episode grid."""
    ...
    output = tuple(state for state in full
                   if lower <= state.decision_node.decision_time <= current_time)
    validate_history_sequence(output, require_episode_start=False)
    return output


def represent_history(states: tuple[PREState, ...] | list[PREState],
                      representation: HistoryRepresentation | str,
                      *, window_minutes: int | None = None) -> tuple[PREState, ...]:
