...
def test_m4_policy_uses_declared_parameters_and_preserves_both_execution_gates(tmp_path: Path):
    ...
    assert artifact["m4_execution_status"] == "M4_TAIL_ASSUMPTION_GROUNDED_READY"
    assert artifact["monetary_mapping_status"] == "FROZEN_ASSUMPTION_GROUNDED"
    assert artifact["monetary_mapping_reference"] == (
        "registries/m4_eur_mapping_assumption_grounded_v1.json"
    )
    assert artifact["monetary_mapping_registry_hash"].startswith("sha256:")
    assert artifact["monetary_mapping_pending_anchor_components"] == (
        "P_itinerary",
        "P_service",
    )
    ranking = artifact["m4_ranking_definition"]
    ...
    assert ranking["registry_hash"] == artifact["monetary_mapping_registry_hash"]
    ...
    assert artifact["policy"]["tail_support_state"] == "FROZEN_ASSUMPTION_GROUNDED"
