...
def _responsibilities(
    path: Path, text: str, symbols: tuple[str, ...]
) -> tuple[str, ...]:
    values = []
    lower = text.lower()
    if "pydantic" in lower or any(
        name.endswith(("Contract", "Request", "Result")) for name in symbols
    ):
        values.append("contracts")
    if any(
        token in lower for token in ("read_text(", "write_text(", "read_csv", "open(")
    ):
        values.append("io")
    ...
    if any(
        token in lower for token in ("validate", "contracterror", "raise valueerror")
    ):
...
def scan_code_structure(root: Path) -> list[dict]:
    ...
    for rel, (text, tree, imports) in sorted(parsed.items()):
        ...
        split_candidate = (
            loc > 800
            or (loc > 500 and len(responsibilities) >= 3)
            or (loc > 300 and len(responsibilities) >= 4)
        )
        ...
        risk = (
            "HIGH"
            if status == "REFACTOR_REQUIRED"
            else "MEDIUM" if status == "REFACTOR_RECOMMENDED" else "LOW"
        )
        records.append(
            {
                "path": rel,
                ...
                "side_effects": any(
                    token in text for token in ("write_text(", "mkdir(", "print(")
                ),
                "io_behavior": (
                    "READ_WRITE"
                    if "write_text(" in text
                        ...
                        if any(
                            token in text
                            for token in ("read_text(", "read_csv", "open(")
                        )
                        else "PURE_OR_IN_MEMORY"
                    )
                ),
                "config_access": (
                    "DIRECT"
                    if "load_config" in text or "foundation.yaml" in text
                    else "NONE"
                ),
                "registry_access": (
                    "DIRECT"
                    if "load_registry" in text or "yaml.safe_load" in text
                    else "NONE"
                ),
