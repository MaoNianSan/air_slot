...
def build_run_report(
    ...
    lines.extend(
        [
            ...
            f"- `AMBIGUOUS_CHAIN_EXCLUDED={'YES' if chains['ambiguous_engineering_eligible'] == 0 else 'NO'}`",
            ...
            f"- `NATIVE_RATIO_DEPENDENCY_ABSENT={'YES' if not observations['ratio_dependency_columns'] else 'NO'}`",
            f"- `ON_DEMAND_EVIDENCE_SUPPORTED={'YES' if observations['on_demand_evidence_supported'] else 'NO'}`",
            f"- `STATE_CACHE_REUSED_WHERE_VALID={'YES' if cache.get('cache_status') == 'V2_HIT' else 'NO_VALID_FULL_REUSE'}`",
