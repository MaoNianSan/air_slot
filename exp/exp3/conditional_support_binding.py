...
RMB_INTERFACE = Path("artifacts/diagnostics/m2_rmb_consequence_mapping_correction_v1/M2_RMB_CONSEQUENCE_INTERFACE.json")
...
def bind(*, root: Path, output_root: Path | None = None) -> dict[str, Path]:
    ...
    output_root = (output_root or root / "artifacts/diagnostics/exp3_conditional_support_binding_v2").resolve()
    ...
    if rmb_interface["status"] != "M2_CONSEQUENCE_TO_RMB_INTERFACE_MATERIALIZED":
        raise RuntimeError("M3_CONDITIONAL_BINDING_RMB_INTERFACE_INVALID")
    if rmb_interface["monetary_mapping_contract"]["formula"] != "RMB_k = f_k(C_k)":
        raise RuntimeError("M3_CONDITIONAL_BINDING_RMB_FORMULA_INVALID")
    payload = {
        "schema_version": "EXP3_CONDITIONAL_SUPPORT_BINDING_V1",
    ...
    manifest = {"schema_version": "EXP3_CONDITIONAL_SUPPORT_BINDING_MANIFEST_V1", "status": payload["status"], "artifact": str(artifact_path.resolve()), "artifact_hash": payload["artifact_hash"], "conditional_action_count": 22, "formal_multi_action_status": "BLOCKED_UNCHANGED", "monetary_chain": "C^a -> RMB^a -> risk", "safety": dict(SAFETY)}
