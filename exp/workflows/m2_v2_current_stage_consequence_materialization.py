...
Path-B (G1): the seven-component ontology is preserved exactly and the frozen
``M2_DATA2_FORMAL_CU_V2`` normalization receives constructed loss units.
``P_itinerary`` and ``P_service`` carry ASSUMPTION_GROUNDED (literature-
parameterized) inputs, so the seven-component aggregate is materialized as an
assumption-grounded layer; the formal/empirical aggregate remains unresolved
and no monetary claim is made.
"""
...
ASSUMPTION_FREEZE_CLOSURE = Path(
    "artifacts/diagnostics/v5_development_freeze/M2_FORMAL_FREEZE_CLOSURE_V2.json"
)
...
def load_assumption_freeze_id(root: Path) -> str:
    """Return the frozen V2 closure registry hash used as assumption freeze id."""
    closure = _load(root / ASSUMPTION_FREEZE_CLOSURE)
    freeze_id = closure.get("registry_hash")
    _require(bool(freeze_id), "M2_V2_ASSUMPTION_FREEZE_ID_MISSING")
...
def _compact(consequence: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "seven_component_reason": None if seven_supported else "P_ITINERARY_OR_P_SERVICE_ASSUMPTION_UNSUPPORTED",
...
def materialize(
    *,
    ...
    freeze_id = load_assumption_freeze_id(root)
    for node_id, source_rows in by_node.items():
        airport_keys = node_airports[node_id]
        context = build_assumption_grounded_context(
            ...
            assumption_freeze_id=freeze_id,
        )
    ...
    payload = {
        ...
        "status": "M2_V2_SEVEN_COMPONENT_REPRESENTATION_MATERIALIZED_ASSUMPTION_GROUNDED",
        ...
        "cu_normalization": {
            ...
            "role": "SEVEN_COMPONENT_ASSUMPTION_GROUNDED_SUM_OVER_SEVEN_ONLY_IF_ALL_SUPPORTED",
        },
        ...
        "representation_readiness": {
            ...
            "EXP2B_7COMP_TYPED_VECTOR": "READY_TYPED_VECTOR_ASSUMPTION_GROUNDED",
            "EXP2B_3CHANNEL": "READY_ASSUMPTION_GROUNDED",
            "EXP2B_SCALAR": "READY_ASSUMPTION_GROUNDED_RANKING_GATED",
        },
        "next_gate": "M4_MATERIAL_COVERAGE_FREEZE_AND_FINAL_TEST_AUTHORIZATION",
