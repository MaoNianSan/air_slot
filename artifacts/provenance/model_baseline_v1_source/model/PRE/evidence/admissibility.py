...
def latest_legal(
    ...
    *,
    ...
    legal = [
        candidate for candidate in candidates if candidate.availability_time <= cutoff
    ]
    if not legal:
        raise ContractError("NO_LEGAL_EVIDENCE")
    latest_time = max(candidate.availability_time for candidate in legal)
    latest = [
        candidate for candidate in legal if candidate.availability_time == latest_time
    ]
