...
class ProductionPREPublisher:
    ...
    def target_support(
        self, dataset_instance_id: str
    ) -> tuple[TargetSupportState, ...]:
    ...
    def publish(self, request: ProductionPRERequest) -> PREBuildResult:
        ...
        provisional = build_decision_node(
            ...
            legal_record_ids=(),
            operational_stage=request.operational_stage,
            node_index=request.node_index,
            roll_minutes=request.roll_minutes,
        )
        mapped_records = tuple(
            item
            for item in (mapper.map_record(record) for record in request.records)
            if item is not None
        )
        ...
        airport_roles = (
            ...
            if schedule is None
                ...
                "connection": request.connection_airport_id
                or schedule.get("origin_airport_id"),
            }
        )
        ...
        ledger = tuple(
            item.model_copy(update={"decision_node_id": node.decision_node_id})
            for item in ledger
        )
        lineage = tuple(
            item.model_copy(update={"decision_node_id": node.decision_node_id})
        ...
        provisional_pre = PREState(
            decision_node=node,
            predecessor_state=families["predecessor_state"],
        ...
        published, publication_meta = publish_static_reference(
            provisional_pre,
            taxi_reference=request.taxi_reference,
        ...
        return PREBuildResult(
            pre_state=PREState(
                ...
                reference_state=ReferenceState(entries=families["reference_state"]),
                target_support=target_support,
                static_reference_publication=publication_meta,
            ),
            FIXTURE_ONLY=False,
