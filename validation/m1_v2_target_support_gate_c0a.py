...
def _episode_values(
    groups: dict[str, list[float]], target: str
    ...
    for episode_id, items in groups.items():
        ...
        values[episode_id] = (
            max(items) if target == "T_IB_REMAINING_HAZARD" else unique[0]
        )
    return values, {
        ...
        "assertion": (
            "WITHIN_EPISODE_VALUE_EXACT"
            if target in {"D_OB", "D_TX"}
            else "T_IB_NODE_DEPENDENT_MAX"
        ),
    }


def _candidate_rows(
    row_values: Iterable[float], episode_values: Iterable[float], target: str
) -> list[dict[str, Any]]:
    ...
    max_value = max(
        float(np.max(row_array)) if len(row_array) else 0.0,
        ...
        rows.append(
            {
                ...
                "episode_finite_count": int(np.sum(~ep_tail)),
                "episode_tail_count": int(np.sum(ep_tail)),
                "episode_tail_fraction": (
                    float(np.mean(ep_tail)) if len(episode_array) else None
                ),
                "row_finite_count": int(np.sum(~row_tail)),
                "row_tail_count": int(np.sum(row_tail)),
                "row_tail_fraction": (
                    float(np.mean(row_tail)) if len(row_array) else None
                ),
                "additional_finite_classes": max(
                    0, candidate // BIN_WIDTH - current // BIN_WIDTH
                ),
                "observed_additional_finite_bins": len(
                    {int(v // BIN_WIDTH) for v in episode_array if v < candidate}
...
def _lineage_fields(lineage: dict[str, Any] | None) -> dict[str, Any]:
    lineage = lineage or {}
    schedule, route, carrier, taxi = (
        lineage.get("schedule_reference") or {},
    ...
    return {
        ...
        "taxi_reference_hash": taxi.get("freeze_id"),
...
def _source_status_for_episode(
    ...
    predecessor, successor = records.get(episode.predecessor_flight_id), records.get(
        episode.successor_flight_id
    )
    if predecessor is None or successor is None:
        return {
            "status": "SOURCE_VERIFICATION_REQUIRED",
            "reason": "SELECTED_CANONICAL_RECORD_MISSING",
        }
    result = {
        ...
        "row_repetitions": len(values),
        "target_values": sorted(set(values)),
    ...
    if target == "T_IB_REMAINING_HAZARD":
        expected_max = max(
            0.0,
            (
                predecessor["actual_arrival_utc"] - episode.episode_start_time
            ).total_seconds()
            / 60.0,
        )
        ...
        actual, scheduled = (
            ...
            successor["schedule_departure_utc"],
        )
        ...
        result.update(
            {
                "actual_departure_utc": actual,
                "scheduled_departure_utc": scheduled,
                "derived_d_ob_minutes": derived,
                "label_match": all(abs(derived - value) <= 1e-6 for value in values),
                "signed_dep_delay": successor["DepDelay"],
                "direct_clock_classification": clock["classification"],
                "local_wall_clock_residual_minutes": clock[
                    "local_wall_clock_residual_minutes"
                ],
        ...
        derived = (
            None if ref is None or taxi is None else max(0.0, float(taxi) - float(ref))
        )
        result.update(
            {
                "taxi_out_minutes": taxi,
                "taxi_reference_minutes": ref,
                ...
                "taxi_reference_id_match": lineage.get("taxi_reference_id")
                == taxi_identity["reference_id"],
                "taxi_reference_hash_match": lineage.get("taxi_reference_hash")
                == taxi_identity["manifest_freeze_id"],
                "derived_d_tx_minutes": derived,
                "label_match": derived is not None
                and all(abs(derived - value) <= 1e-6 for value in values),
        ...
        if not all(
            (
                result["label_match"],
                result["taxi_reference_id_match"],
                result["taxi_reference_hash_match"],
                bool(result["taxi_reference_level"]),
            )
        ):
            result["status"] = "SOURCE_CONFLICT"
    return result
...
def _episode_audit(
    ...
    taxi_payload = _read_json(
        A2_OUTPUT / "DATA2_TAXI_REFERENCE_GATE_A2_DIAGNOSTIC.json"
    )
    taxi_identity = {
        key: taxi_payload[key] for key in ("reference_id", "manifest_freeze_id")
    }
    ...
    for split in SPLITS:
        ...
        for target in TARGETS:
            row_values = [
                value for _, value in _active_label_rows(cache, target, split)
            ]
            ...
            all_profiles[split][target] = {
                ...
                "episode_profile": _profile(
                    episode_values.values(), CURRENT_SUPPORT[target]
                ),
                "row_active_count": len(row_values),
                "unique_episode_count": len(episode_values),
                "within_episode_value_consistency": consistency,
                "episode_values": {
                    key: float(value) for key, value in episode_values.items()
                },
            }
            if split == "train":
                all_candidates.extend(
                    _candidate_rows(row_values, episode_values.values(), target)
                )
            for episode_id, value in episode_values.items():
                ...
                indices = [
                    i
                    for i, row_value in _active_label_rows(cache, target, split)
                    if store.sample_episode_ids[i] == episode_id
                ]
                lineage = _lineage_fields(
                    store.static_context_lineages[indices[0]] if indices else None
                )
                ...
                row = {
                    ...
                    "target_value": value,
                    "row_repetition_count": len(groups[episode_id]),
                    "origin": lineage.get("origin"),
                    "destination": lineage.get("destination"),
                    "carrier": lineage.get("carrier"),
                    "source_verification": "SOURCE_VERIFICATION_REQUIRED",
                }
    ...
    return {
        ...
        "overflow_episodes": overflow_rows,
...
def _weighting_audit() -> dict[str, Any]:
    ...
    uses_weights = (
        "episode_normalized_weights" in lifecycle
        and re.search(r"episode_weights|weights=.*episode", lifecycle) is not None
    )
    return {
        "episode_balanced_loss_status": (
            "PASS" if uses_weights else "M1_EPISODE_WEIGHTING_CONTRACT_REVIEW_REQUIRED"
        ),
        ...
        "evidence": (
            ...
            if not uses_weights
            else "episode-normalized weights are consumed by M1Lifecycle."
        ),
    }
...
def _recommendations(audit: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    d_ob_210 = next(
        row
        for row in audit["candidate_tables"]
        if row["target"] == "D_OB" and row["candidate_support_minutes"] == 210
    )
    d_ob = [
        row
        for row in audit["overflow_episodes"]
        if row["target"] == "D_OB" and row["split"] == "train"
    ]
    ...
    return [
        {
            ...
            "recommendation": "KEEP_360" if tib_tail <= 1 else "EXPAND_TO_390",
        ...
        },
        {
            ...
            "recommendation": (
                "EXPAND_TO_210"
                if len(d_ob) >= 2
                and d_ob_210["episode_tail_count"]
                < dob["episode_profile"]["current_support_count"]
                else "KEEP_180"
            ),
        ...
        },
        {
            "decision_id": "C0A-D03",
            "target": "D_TX",
            "recommendation": "KEEP_60" if dtx_tail == 0 else "EXPAND_TO_75",
...
def _write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for row in rows:
            writer.writerow(
                {
                    key: "" if value is None else _jsonable(value)
...
def _packet(report: dict[str, Any]) -> str:
    ...
    for target in TARGETS:
        ...
        lines.append(
            f"- `{target}`: rows={train['row_active_count']}, unique episodes={train['unique_episode_count']}, episode profile={json.dumps(train['episode_profile'], sort_keys=True)}"
        )
    lines.extend(
        [
            "",
    ...
    for row in report["episode_balance"]["overflow_episodes"]:
        lines.append(
            f"| {row['split']} | {row['target']} | {row['target_value']} | {row['row_repetition_count']} | {row['source_verification']} |"
        )
    lines.extend(
        [
            "",
            "## Training Weighting",
            "",
            f"- `{report['training_weighting']['episode_balanced_loss_status']}`",
            f"- {report['training_weighting']['evidence']}",
            "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- B2 schema: `{report['b2_immutability']['schema_hash']}`",
            f"- B2 cache: `{report['b2_immutability']['cache_hash']}`",
            f"- Labels unchanged: `{report['b2_immutability']['labels_unchanged']}`",
            f"- Active masks unchanged: `{report['b2_immutability']['active_masks_unchanged']}`",
            "",
            "```text",
            "M1_TRAINING_RUNS = 0",
            "TUNING_RUNS = 0",
...
def run(
    output_dir: Path = DEFAULT_OUTPUT, *, source_scan: bool = True
) -> dict[str, Any]:
    ...
    source = (
        scan_source_clock(cache, episodes)
        if source_scan
        else {
            "scope": "SOURCE_SCAN_SKIPPED_FOR_UNIT_TEST",
            "departure_inconsistency_count": None,
            "departure_classification_counts": {},
            "departure_inconsistency_rows": [],
            "intersection": {},
            "canonical_timestamp_change_count": None,
            "canonical_timestamp_change_count_in_b2": None,
            "max_difference_case": None,
            "selected_canonical_records": {},
            "final_test_access_count": 0,
        }
    )
    if source_scan:
    ...
    contract_failure = (
        not schema_unchanged
        or not cache_unchanged
        or labels_before != labels_after
        or active_before != active_after
        or any(
            item["within_episode_value_consistency"]["status"] != "PASS"
            for split in balance["profiles"].values()
            for item in split.values()
        )
        or any(
            item["status"] != "SOURCE_CONSISTENT"
            for item in balance["source_verification"]
        )
    ...
    report = {
        ...
        "TARGET_SUPPORT_C0A_STATUS": (
            "TARGET_SUPPORT_C0A_REVIEW_PACKET_READY" if not flags else flags[0]
        ),
        "status_flags": flags,
        "repository_head": _head(),
        "scope": "SOURCE_VERIFICATION_EPISODE_BALANCE_HUMAN_REVIEW_ONLY",
        ...
        "b2_immutability": {
            ...
            "schema_expected": EXPECTED_SCHEMA_HASH,
            "cache_expected": EXPECTED_CACHE_HASH,
            "schema_unchanged": schema_unchanged,
            "cache_unchanged": cache_unchanged,
            "labels_before": labels_before,
            "labels_after": labels_after,
            "active_masks_before": active_before,
            "active_masks_after": active_after,
    ...
    _write_json(
        output_dir / "M1_V2_TARGET_SUPPORT_GATE_C0A_EPISODE_PROFILES.json",
        balance["profiles"],
    )
    _write_json(
        output_dir / "M1_V2_TARGET_SUPPORT_GATE_C0A_SOURCE_RECORDS.json", source_records
    )
    _write_csv(
        output_dir / "M1_V2_TARGET_SUPPORT_GATE_C0A_SOURCE_CLOCK.csv",
        source_report["departure_inconsistency_rows"],
    )
    _write_csv(
        output_dir / "M1_V2_TARGET_SUPPORT_GATE_C0A_EPISODE_THRESHOLDS.csv",
        balance["candidate_tables"],
    )
    _write_csv(
        output_dir / "M1_V2_TARGET_SUPPORT_GATE_C0A_OVERFLOW_EPISODES.csv",
        balance["overflow_episodes"],
    )
    (output_dir / "M1_V2_TARGET_SUPPORT_GATE_C0A_HUMAN_REVIEW_PACKET.md").write_text(
        _packet(report), encoding="utf-8"
    )
    return report
