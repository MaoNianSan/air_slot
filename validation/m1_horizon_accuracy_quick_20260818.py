...
def nearest_horizon(values: np.ndarray) -> np.ndarray:
    ...
    """
    boundaries = np.array(
        [(HORIZONS[i] + HORIZONS[i + 1]) / 2.0 for i in range(len(HORIZONS) - 1)]
    )
...
def encode_vectorized(values: np.ndarray, contract: _Bin) -> np.ndarray:
    ...
    if contract.signed:
        ...
        finite = np.where(
            values >= contract.finite_max + contract.width,
            contract.overflow_index,
            finite,
        )
        out[...] = finite
    else:
        ...
        enc = np.where(
            values >= contract.finite_max + contract.width, contract.overflow_index, enc
        )
        out[...] = enc
    return out
...
def nll_binned(
    draws: np.ndarray, realized_label: np.ndarray, contract: _Bin
...
def main() -> None:
    ...
    assert (
        nodes["sample_index"].to_numpy() == dev_idx
    ).all(), "node.parquet order != cache dev order"
    cache_node_ids = z["sample_decision_node_ids"][dev_idx]
    assert (
        nodes["decision_node_id"].to_numpy() == cache_node_ids
    ).all(), "node id mismatch"
    ...
    node_order = {
        node_id: i for i, node_id in enumerate(nodes["decision_node_id"].to_numpy())
    }
    scenarios["node_idx"] = (
        scenarios["decision_node_id"].map(node_order).astype(np.int32)
    )
    ...
    # ---- realized horizons -------------------------------------------------
    lead_to_off_block = (
        nodes["scheduled_ob_utc"] - nodes["decision_time"]
    ).dt.total_seconds().to_numpy() / 60.0 + realized["DELTA_OB"]
    ...
    ).dt.total_seconds().to_numpy() / 60.0 + nodes.loc[
        ~active["DELTA_OB"], "observed_delta_ob"
    ].to_numpy(
        dtype=np.float64
    )
    ...
    horizon_allowed = {
        ...
        "T_TX": active["T_TX"]
        & (nodes["operational_stage"].to_numpy() != "POST_OB_PRE_TO"),
        "D_TO": active["T_TX"]
        & (nodes["operational_stage"].to_numpy() != "POST_OB_PRE_TO"),
    }
    ...
    realized_d_to[~active["T_TX"]] = (
        np.nan
    )  # D_TO only meaningful where joint available
    ...
    for name in TARGETS:
            ...
            rows.append(
                {
                    ...
                    **{
                        f"acc_within_{tol}_min": float(np.mean(ae <= tol))
                    ...
                    "realized_overflow_count": int(
                        tail_flags[name]["overflow"][mask].sum()
                    ),
                    "realized_underflow_count": int(
                        tail_flags[name]["underflow"][mask].sum()
                    ),
                }
            )
        ...
        support.update(
            {
                ...
                "note": (
                    "all finite realized horizons are assigned to a bucket by the deterministic "
        ...
        output["targets"][name] = {
            ...
            "overall": {
                ...
                **{
                    f"acc_within_{tol}_min": float(np.mean(ae_o <= tol))
                    for tol in TOLERANCES
                },
            },
            "horizons": rows,
    ...
    output["targets"]["D_TO"] = {
        ...
        "overall": {
            ...
            **{
                f"acc_within_{tol}_min": float(
                    np.mean(np.abs(dt_err[dt_allowed]) <= tol)
                )
                for tol in TOLERANCES
            },
        },
    ...
    fixed_threshold = (
        0.384  # frozen Exp1 FIXED_HISTORY operating point at target FPR 0.1
    )
    ...
    for name in ("R_IB", "DELTA_OB", "T_TX"):
        ...
        print(
            ...
            "overall:",
            {
                ...
                for k, v in item["overall"].items()
            },
        )
        for row in item["horizons"]:
            print(
                "  H=%3d N=%4d MAE=%6.2f RMSE=%6.2f ±10=%.3f ±15=%.3f ±30=%.3f cov50=%.3f cov80=%.3f cov90=%.3f"
                % (
                    row["horizon_minutes"],
                    row["N"],
                    row["MAE_min"],
                    row["RMSE_min"],
                    row["acc_within_10_min"],
                    row["acc_within_15_min"],
                    row["acc_within_30_min"],
                    row["cov50"],
                    row["cov80"],
                    row["cov90"],
                )
