...
class FootprintRole(str, Enum):
    """Structural consequence role for one action/component cell."""
...
class FootprintLevel(str, Enum):
    """Structural relevance level; not a numerical effect magnitude."""
...
class ActionFootprint(FrozenModel):
    role: FootprintRole
    level: FootprintLevel

    @model_validator(mode="after")
    def role_level_consistency(self):
        if self.role is FootprintRole.UNTOUCHED and self.level is not FootprintLevel.NONE:
            raise ValueError("UNTOUCHED_FOOTPRINT_REQUIRES_NONE_LEVEL")
        if self.role is not FootprintRole.UNTOUCHED and self.level is FootprintLevel.NONE:
            raise ValueError("ACTIVE_FOOTPRINT_REQUIRES_NON_NONE_LEVEL")
        return self
...
class ActionTemplate(FrozenModel):
    ...
    footprint: dict[str, ActionFootprint] = {}
    ...
    def strict_contract(self):
        ...
        if set(self.footprint) - set(CONSEQUENCE_COMPONENTS):
            raise ValueError("UNKNOWN_ACTION_FOOTPRINT_COMPONENT")
        for component, value in self.mitigation.items():
            if not isfinite(float(value)) or not 0.0 <= float(value) <= 1.0:
                raise ValueError(f"INVALID_MITIGATION_COEFFICIENT:{component}")
        for component, value in {**self.induced, **self.induced_response}.items():
            if not isfinite(float(value)) or float(value) < 0.0:
                raise ValueError(f"INVALID_INDUCED_COEFFICIENT:{component}")
        for component in self.mitigation:
            cell = self.footprint.get(component)
            if cell is not None and cell.role is not FootprintRole.MITIGATION:
                raise ValueError(f"MITIGATION_FOOTPRINT_ROLE_MISMATCH:{component}")
        for component in {**self.induced, **self.induced_response}:
            cell = self.footprint.get(component)
            if cell is not None and cell.role is not FootprintRole.INDUCED:
                raise ValueError(f"INDUCED_FOOTPRINT_ROLE_MISMATCH:{component}")
        for component, cell in self.footprint.items():
            if cell.role is FootprintRole.UNTOUCHED and (
                component in self.mitigation or component in self.induced or component in self.induced_response
            ):
                raise ValueError(f"UNTOUCHED_FOOTPRINT_HAS_COEFFICIENT:{component}")
        ...
        if self.template_id == "A00":
            ...
            if self.mitigation or self.induced or self.induced_response:
                raise ValueError("A00_MUST_HAVE_NO_EFFECT_COEFFICIENTS")
            if self.footprint and any(
                item.role is not FootprintRole.UNTOUCHED
                or item.level is not FootprintLevel.NONE
                for item in self.footprint.values()
            ):
                raise ValueError("A00_FOOTPRINT_MUST_BE_IDENTITY")
        elif (
