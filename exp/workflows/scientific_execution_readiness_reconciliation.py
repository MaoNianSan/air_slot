...
def _inputs(root: Path) -> dict[str, Path]:
    paths = {
        ...
        "exp3_manifest": root / "artifacts/diagnostics/exp3_formal_execution_preparation_v11/EXP3_FORMAL_EXECUTION_MANIFEST.json",
        "exp3_readiness": root / "artifacts/diagnostics/exp3_formal_execution_preparation_v11/EXP3_FORMAL_EXECUTION_READINESS_REPORT.json",
        "exp4_manifest": root / "artifacts/diagnostics/exp4_formal_execution_preparation_v11/EXP4_FORMAL_EXECUTION_MANIFEST.json",
        "exp4_readiness": root / "artifacts/diagnostics/exp4_formal_execution_preparation_v11/EXP4_FORMAL_EXECUTION_READINESS_REPORT.json",
    ...
    claims = {
        "claims": [
            ...
            },
            {
                ...
                "evidence_boundary": "CU material coverage is frozen ASSUMPTION_GROUNDED; the five-anchor constructed-EUR ranking is assumption-grounded and P_itinerary/P_service per-event monetary anchors remain a human gate",
...
def _minimum_artifacts(fixed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "Exp4": {
            ...
            "current_status": fixed["Exp4"]["baseline_status"],
        },
    }


def _exp4_baseline_status(root: Path, paths: dict[str, Path]) -> str:
    """FOUR-ARTIFACT binding status from the Exp4 full-development manifest."""
    execution_manifest = root / "artifacts/experiments/exp4/full_development_v1/EXP4_FULL_DEVELOPMENT_EXECUTION_MANIFEST.json"
    if not execution_manifest.is_file():
        return "BLOCKED_CURRENT_ARTIFACT_AND_BASELINE_GATES"
    manifest = _load(execution_manifest)
    model_artifacts = manifest.get("model_artifacts", {})
    if set(model_artifacts) != {"HISTORICAL", "LIGHTGBM", "RANDOM_FOREST", "STATE_AWARE_H32"}:
        return "BLOCKED_BASELINE_ARTIFACT_SET_INCOMPLETE"
    for baseline, entry in model_artifacts.items():
        artifact = root / entry["path"]
        if not artifact.is_file():
            return "BLOCKED_BASELINE_ARTIFACT_FILE_MISSING"
        if _hash(artifact) != entry["sha256"]:
            return "BLOCKED_BASELINE_ARTIFACT_HASH_MISMATCH"
    return "EXP4_BASELINES_FROZEN_BOUND_READY"


def _current_status(paths: dict[str, Path], root: Path) -> dict[str, Any]:
    ...
    m2_registry = _load(paths["m2_registry"])
    return {
        "M1": {
            "status": m1["status"],
        ...
        "M3": {
            ...
            "action_set_frozen": m3.get("action_set_frozen", {}).get("status", "NOT_FROZEN"),
        },
        "M4": {
            ...
            "material_coverage_status": (
                "FROZEN_ASSUMPTION_GROUNDED"
                if (
                    m2_registry.get("aggregation_rule") == "SUM_OVER_SEVEN_ONLY_IF_ALL_SUPPORTED"
                    and m2_registry.get("assumption_grounded", {}).get("policy")
                    == "ASSUMPTION_GROUNDED_NOT_EMPIRICAL"
                    and len(m2_registry.get("component_weights", {})) == 7
                )
                else "UNFROZEN"
            ),
        },
        ...
        "Exp4": {
            ...
            "baseline_status": _exp4_baseline_status(root, paths),
            "baselines": _load(paths["exp4_manifest"]).get("baselines", []),
        },
    }


def _full_chain_status(root: Path) -> str:
    """Development full-chain materialization status for Exp1-Exp4 official runs."""
    required = (
        root / "artifacts/experiment/exp1_full_development/EXP1_FULL_DEVELOPMENT_EXECUTION_MANIFEST.json",
        root / "artifacts/experiments/exp2/full_development_v1/EXP2_FULL_DEVELOPMENT_EXECUTION_MANIFEST.json",
        root / "artifacts/experiments/exp3/full_development_v1/EXP3_FULL_DEVELOPMENT_EXECUTION_MANIFEST.json",
        root / "artifacts/experiments/exp4/full_development_v1/EXP4_FULL_DEVELOPMENT_EXECUTION_MANIFEST.json",
    )
    if not all(path.is_file() for path in required):
        return "BLOCKED_UPSTREAM"
    return "COMPLETED_DEVELOPMENT_FULL_CHAIN_MATERIALIZED"


def _shortest_path(current: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "step": 1,
        ...
        },
        {
            ...
            "owner": "M2",
            "status": "COMPLETED_WITH_ASSUMPTION_GROUNDED_SUPPORT",
            "gate": "Seven CU components are materialized; P_itinerary and P_service carry ASSUMPTION_GROUNDED literature-parameterized provenance",
        ...
        },
        {
            ...
            "owner": "M3",
            "status": "COMPLETED_EXP3_FORMAL_COHORT_ASSUMPTION_GROUNDED_READY",
            "gate": "22 non-A00 actions carry ASSUMPTION_GROUNDED mechanism responses executable in the SCENARIO/CONDITIONAL lane; FORMAL/authoritative ranking remains gated",
        ...
        },
        {
            ...
            "gate": "formal_support_upgrade=false; six mechanism families covered (AIRCRAFT_SWAP/CREW_SWAP/HOLD/PROPAGATION_BUFFER/PASSENGER_REASSIGNMENT/NETWORK_EXTREME) with documented orthogonality check table (M3_ACTION_SET_FROZEN_V1)",
        ...
        },
        {
            ...
            "owner": "M4",
            "status": "COMPLETED_ASSUMPTION_GROUNDED_CU_MATERIAL_COVERAGE_FROZEN",
            "gate": "CU material coverage frozen ASSUMPTION_GROUNDED (unit weights + 0.5/1.0/2.0 sensitivity); complete seven-component monetary ranking remains a human gate (P_itinerary/P_service per-event anchors)",
            "unlocks": ["Exp2 assumption-grounded ranking", "Exp3 assumption-grounded coverage/ablations", "Exp4 m4 lanes under assumption grounding"],
        },
        {
            "step": 8,
            "action": "Bind_Exp4_predictive_baseline_and_Data1_typed_evaluation_artifacts",
            "owner": "Exp4",
            "status": current["Exp4"]["baseline_status"],
        ...
        },
        {
            ...
            "owner": "Experiments",
            "status": current["full_chain_status"],
...
def reconcile(*, root: Path, output_root: Path | None = None) -> dict[str, Path]:
    ...
    output_root = (output_root or root / "artifacts/diagnostics/exp2_4_scientific_execution_readiness_reconciliation_v21").resolve()
    paths = _inputs(root)
    ...
    current = _current_status(paths, root)
    current["full_chain_status"] = _full_chain_status(root)
    ...
    source_manifest = _artifact({
        ...
        "shortest_path": _shortest_path(current),
        "prohibitions": {
            "Final_Test": True,
            "paper_full": True,
    ...
    readiness = _artifact({
        ...
        "exp2_status": "TAIL_AWARE_BRIER_AND_CALIBRATION_MATERIALIZED_RANKING_ASSUMPTION_GROUNDED_READY",
        "exp3_status": current["Exp3"]["execution_status"],
        "exp4_status": current["Exp4"]["baseline_status"],
        "first_automatic_action": "EXECUTE_ASSUMPTION_GROUNDED_5_ANCHOR_SUBSET_RANKING_EXP3_ABLATIONS_DEFERRED_BY_USER",
        "first_human_gate": "MONETARY_MAPPING_NUMERIC_ANCHOR_FREEZE_OR_FINAL_TEST_AUTHORIZATION_REQUIRED",
        "human_decisions": [{
            "date": "2026-08-24",
            "subject": "Exp3 variants/ablations deferral",
            "variants": ["MODULE_REMOVAL_*", "ROLLING", "ONE_SHOT", "SYNC", "LAG_*"],
            "status": "DEFERRED_OPTIONAL",
            "paper_inclusion": False,
            "reason": "User decision 2026-08-24: do not re-run; not in paper; no frozen artifact change.",
        }],
        ...
        "blocker_count": sum(item["status"].startswith("BLOCKED_") for item in _shortest_path(current)),
        "safety": dict(_SAFETY),
    })
    ...
    manifest = _artifact({
        ...
        "m3_binding": {
            ...
            "non_a00_v2_execution_enabled": current["M3"]["non_a00_v2_execution_enabled"],
        },
        "m4_binding": {
            "material_coverage_status": current["M4"]["material_coverage_status"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    reconcile(root=Path(__file__).resolve().parents[2], output_root=args.output_root)
