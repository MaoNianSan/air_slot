...
ColumnRole = Literal[
    ...
    "DIAGNOSTIC_ONLY",
    "SIGNED_TIME_OFFSET",
    "NONNEGATIVE_DELAY_REPORTING_ONLY",
...
class DataUsageRule(FrozenModel):
    ...
    fallback_rule: str
    pre_family: str
    downstream_consumers: tuple[Consumer, ...]
    ...
    def enforce_support_ceiling(self):
        ...
        if self.source_kind == "RAW_SOURCE":
            if self.source_rule_id or self.derived_artifact_schema:
                raise ValueError(
                    "raw source cannot declare projection/artifact ownership"
                )
        elif self.source_kind == "PROJECTION":
            if not self.source_rule_id or not self.projection_role:
                raise ValueError(
                    "projection requires source_rule_id and projection_role"
                )
            if self.raw_columns:
...
class DatasetCapability(FrozenModel):
    ...
    decision_time_role: DecisionTimeRole
    max_evidence_class: EvidenceClass
    formal_input_support: EvidenceClass
    realized_outcome_support: EvidenceClass
    freeze_state: FreezeState
    reason_code: str | None = None
    ...
    def explicit_unsupported(self):
        states = (
            self.max_evidence_class,
            self.formal_input_support,
            self.realized_outcome_support,
            self.freeze_state,
        )
        if (
            any(value.value == "UNSUPPORTED" for value in states)
