"""D2-9 M1 training coverage: all rolling grid decision nodes as training samples.

Frozen rule DATA2_M1_TRAINING_COVERAGE@1.0.0 / D2-M1-TRAINING-COVERAGE@1.0.0:
one M1 training example per decision node (node-level equal weight), stage-gated
labels, D2-2 anchors unchanged. Episodes whose grid has no PRE_IB node (early
...
supervision. Dataset isolation is enforced by build_data2_target_labels
(M1_DATA2_TARGET_BUILDER_DATASET_MISMATCH).
"""
...
def active_node_prefixes(*, episode: EpisodeRecord,
    ...
    Prefix states run from the grid start (D2-2 anchor: pred.CRSArr) through the
    node; labels are stage-gated by build_data2_target_labels. Nodes whose
    ...
    for index, node in enumerate(nodes):
        labels = build_data2_target_labels(
...
def build_all_node_examples(*, episode: EpisodeRecord,
    ...
    """Build one M1 training example per active node, in node order (D2-9)."""
