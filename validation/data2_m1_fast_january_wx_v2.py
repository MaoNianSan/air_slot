# -*- coding: utf-8 -*-
"""data2 fast-tier run with D2-9 all-node M1 training coverage: BTS 2019-01 only.
...
Weather note (wx_v2, after D2-NOAA-ISD alignment, matrix s30): D2-NOAA-ISD maps to the
current_weather scientific variable (registry canonical_variable=current_weather,
pre_family=current_state).  This run injects the latest legal ISD observation
(availability <= cutoff, age <= weather_max_age=60) for the connection airport
into every PRE publish; airports without a legal observation keep the legal
ABSTAIN semantics (NO_LEGAL_RECORD_AT_DECISION_TIME).

Coverage note (wx_v2, D2-9): training/calibration no longer require a PRE_IB
prefix.  Every rolling grid decision node with >=1 active target becomes one
M1 training example (node-level equal weight); stage-gated labels use the
existing target_builder activation rules; episodes with no PRE_IB node (early
arrival under anchor B) contribute POST_IB_PRE_OB / POST_OB_PRE_TO nodes.
Rule: D2-M1-TRAINING-COVERAGE@1.0.0 (FROZEN).
"""
...
OUT = ROOT / "outputs" / "runtime" / f"data2_m1_fast_january_wx_v2_rl{WEATHER_REPLAY_LAG_MINUTES}"
SEED = 20260813
TRAIN_COUNT = 32
    ...
    for stage in OperationalStage:
        for episode in development_episodes:
            if items[episode.episode_id][0].episode_id in used:
                continue
            match = _pick_prefix(items[episode.episode_id], stage, config_hash, registry_hash, weather_index, weather_max_age)
            if match is None:
                continue
