...
def _position_drift_deg(prev: dict[str, Any], cur: dict[str, Any]) -> float | None:
    if prev.get('latitude_deg') is None or cur.get('latitude_deg') is None \
            or prev.get('longitude_deg') is None or cur.get('longitude_deg') is None:
        return None
    return max(
...
def _drift_per_window(value: float | None, seconds: float, window: float) -> float | None:
    if value is None or seconds <= 0:
        return None
    return value * (window / seconds)


def classify_motion_state(prev: dict[str, Any], cur: dict[str, Any],
                          config: TrajectoryDetectorConfig,
                          next_within_window: dict[str, Any] | None = None) -> tuple[MotionState, tuple[str, ...]]:
    ...
    if on_ground is False:
        ...
        if velocity is not None and float(velocity) >= config.v_taxi_min_mps:
            return MotionState.TAXI, ()
        if next_within_window is not None and next_within_window.get('on_ground') is False:
...
def detect_operational_events(
    ...
    if len(observations) < 2:
        return ()
    ordered = sorted(observations, key=lambda row: (row['aircraft_id'], row['event_time'], str(row.get('_seq', ''))))
        ...
        for index in range(len(rows) - 1):
            ...
            if seconds > config.gap_off_minutes * 60:
                power_off_near = _near_airport(
                    prev.get('latitude_deg'), prev.get('longitude_deg'), airport_reference, config.r_airport_km)
                power_on_near = _near_airport(
                    cur.get('latitude_deg'), cur.get('longitude_deg'), airport_reference, config.r_airport_km)
                if power_off_near:
                    events.append(TrajectoryEventRecord(
                        event_type='POWER_OFF', event_time=prev['event_time'],
                        aircraft_id=aircraft_id, prev_time=prev['event_time'], cur_time=cur['event_time'],
                        prev_state=MotionState.OFF, cur_state=MotionState.OFF,
                    ...
                    events.append(TrajectoryEventRecord(
                        ...
                        aircraft_id=aircraft_id, prev_time=prev['event_time'], cur_time=cur['event_time'],
                        prev_state=MotionState.OFF, cur_state=MotionState.OFF,
                        support_state=SupportState.ABSTAIN, quality_flags=('AIRPORT_RADIUS_OUT_OF_SCOPE',),
                        detector_parameters=config.parameters(),
                        reason_code='POWER_EVENT_AIRPORT_REFERENCE_REQUIRED'))
                if power_on_near:
                    events.append(TrajectoryEventRecord(
                        event_type='POWER_ON', event_time=cur['event_time'],
                        aircraft_id=aircraft_id, prev_time=prev["event_time"], cur_time=cur["event_time"],
                        prev_state=MotionState.OFF, cur_state=MotionState.OFF,
                        support_state=SupportState.SUPPORTED, quality_flags=(),
                        detector_parameters=config.parameters()))
                    ...
                    events.append(TrajectoryEventRecord(
                        ...
                        reason_code='POWER_EVENT_AIRPORT_REFERENCE_REQUIRED'))
                continue
            prev_state, prev_flags = classify_motion_state(
                rows[index - 1] if index > 0 else prev, prev, config) if index > 0 else (MotionState.STATIC, ())
            cur_state, cur_flags = classify_motion_state(prev, cur, config, next_within_window=nxt)
            prev_on_ground = prev.get('on_ground')
            cur_on_ground = cur.get('on_ground')
            primary = None
            if prev_on_ground is not None and cur_on_ground is not None and prev_on_ground != cur_on_ground:
                primary = 'TAKEOFF' if (prev_on_ground is True and cur_on_ground is False) else 'LANDING'
                mismatch = not (
                    (primary == 'TAKEOFF' and prev_state in (MotionState.TAXI, MotionState.STATIC)
                     and cur_state is MotionState.AIR)
                    or (primary == 'LANDING' and prev_state is MotionState.AIR
                ...
                events.append(TrajectoryEventRecord(
                    event_type=primary, event_time=cur['event_time'],
                    aircraft_id=aircraft_id, prev_time=prev['event_time'], cur_time=cur['event_time'],
                    prev_state=prev_state, cur_state=cur_state,
                    support_state=SupportState.DEGRADED if mismatch else SupportState.SUPPORTED,
                    quality_flags=tuple(sorted(set(flags))),
                    detector_parameters=config.parameters(),
                    reason_code='STATE_MACHINE_CROSSCHECK_MISMATCH' if mismatch else None))
                continue
            if prev_on_ground is None or cur_on_ground is None:
                ...
                if inferred is not None:
                    events.append(TrajectoryEventRecord(
                        event_type=inferred, event_time=cur['event_time'],
                        aircraft_id=aircraft_id, prev_time=prev['event_time'], cur_time=cur['event_time'],
                        prev_state=prev_state, cur_state=cur_state,
                        support_state=SupportState.DEGRADED,
                        quality_flags=tuple(sorted(set(prev_flags + cur_flags + ('ONGROUND_MISSING_INFERRED',)))),
                        detector_parameters=config.parameters(),
            ...
            if prev_state is MotionState.STATIC and cur_state is MotionState.TAXI:
                events.append(TrajectoryEventRecord(
                    event_type='OUT_BLOCK_PROXY', event_time=cur['event_time'],
                    aircraft_id=aircraft_id, prev_time=prev['event_time'], cur_time=cur['event_time'],
                    prev_state=prev_state, cur_state=cur_state,
                ...
                events.append(TrajectoryEventRecord(
                    event_type='IN_BLOCK_PROXY', event_time=cur['event_time'],
                    aircraft_id=aircraft_id, prev_time=prev['event_time'], cur_time=cur['event_time'],
                    prev_state=prev_state, cur_state=cur_state,
                    support_state=SupportState.SUPPORTED,
                    quality_flags=tuple(sorted(set(prev_flags + cur_flags))),
