...
from model.M3.readiness import readiness_for_template
...
def evaluate_candidate(
    ...
    *,
    ...
    response_parameter_blocker = None
    if candidate.footprint:
        readiness = readiness_for_template(
            candidate,
            response_status=(
                candidate.response_parameter_status.value
                if hasattr(candidate.response_parameter_status, "value")
                else str(candidate.response_parameter_status)
            ),
        )
        if readiness.missing_response_cells:
            response_parameter_blocker = readiness.reason
    for scenario, open_ in zip(m1_scenarios, opportunities):
        ...
        if candidate.template_id != "A00" and open_:
            if response_parameter_blocker is not None:
                formal_status = FormalEstimandStatus.FORMAL_AGGREGATE_UNRESOLVED
                continue
        ...
        if candidate.template_id == "A00" or not open_:
            ...
            post_cu = action_post_consequences(
                ...
                footprint=candidate.footprint or None,
    ...
    return ActionEvaluation(
        ...
        quality_flags=tuple(
            sorted(
                set(
                    coverage.quality_flags
                    + ((response_parameter_blocker,) if response_parameter_blocker else ())
                )
            )
        ),
        coverage_explanation=tuple(
