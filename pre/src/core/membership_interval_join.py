...
MEMBERSHIP_COLUMNS = [
    "membership_id", "chain_episode_id", "observation_id", "source",
    "flight_id", "request_start", "request_end", "interval_type", "split",
    "membership_role", "availability_supported", "membership_reason",
]
...
def _stable_membership_ids(
    ...
    return [hashlib.sha256(value.encode("utf-8")).hexdigest()[:32] for value in payload]
...
def _vectorized_roles(
    ...
    episode_start = pd.to_datetime(
        request_rows.get("episode_start_time", pd.Series(pd.NaT, index=request_rows.index)),
        utc=True,
    )
    predecessor_end = pd.to_datetime(
        request_rows.get("predecessor_lastseen_proxy", pd.Series(pd.NaT, index=request_rows.index)),
        utc=True,
    )
    successor_start = pd.to_datetime(
        request_rows.get("successor_firstseen_proxy", pd.Series(pd.NaT, index=request_rows.index)),
        utc=True,
    )
    ...
    return np.select(
        [
            ...
            predecessor_end.notna().to_numpy() & events.le(predecessor_end).to_numpy(),
            successor_start.notna().to_numpy() & events.lt(successor_start).to_numpy(),
        ],
...
def interval_join_partition(
    ...
    *,
    ...
    """Join one source/date observation partition to overlapping requests.

    Each identity group is sorted once. ``searchsorted`` locates every request
    slice, and membership columns are built from repeated/taken arrays.
    """
    ...
    required_observation = {
        ...
        "availability_time", observation_identity,
    }
    required_request = {
        "chain_episode_id", "source", "request_start", "request_end",
        "interval_type", "split", request_identity,
    }
    ...
    if missing_request:
        raise ValueError(
            "MEMBERSHIP_REQUEST_IDENTITY_COLUMNS_MISSING=" + ",".join(missing_request)
        )
    ...
    req["request_start"] = pd.to_datetime(req["request_start"], utc=True, errors="coerce")
    req["request_end"] = pd.to_datetime(req["request_end"], utc=True, errors="coerce")
    ...
    request_groups = {
        str(identity): group.sort_values(
            ["request_start", "request_end", "chain_episode_id"], kind="mergesort"
        ).reset_index(drop=True)
        for identity, group in req.groupby(request_identity, sort=False, dropna=False)
        if pd.notna(identity)
    }
    pieces: list[pd.DataFrame] = []
    for identity, obs_group in obs.groupby(observation_identity, sort=False, dropna=False):
        if pd.isna(identity):
            continue
        ...
        left = np.searchsorted(
            event_ns, req_group["request_start"].astype("int64").to_numpy(), side="left"
        )
        right = np.searchsorted(
            event_ns, req_group["request_end"].astype("int64").to_numpy(), side="right"
        )
        lengths = right - left
        valid = lengths > 0
        if not valid.any():
        ...
        obs_positions = np.concatenate(
            [np.arange(left[position], right[position], dtype=np.int64) for position in valid_positions]
        )
        req_positions = np.repeat(valid_positions, lengths[valid])
        ...
        interval_type = selected_req["interval_type"].astype("string")
        ...
        piece = pd.DataFrame(
            {
                ...
                "interval_type": interval_type.to_numpy(),
        ...
        piece.insert(
            ...
            _stable_membership_ids(
                piece["chain_episode_id"], piece["observation_id"], piece["interval_type"]
            ),
        )
