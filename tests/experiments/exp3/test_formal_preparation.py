...
def test_formal_preparation_records_variants_and_keeps_execution_gated(tmp_path):
    ...
    assert readiness["execution_status"] == "EXP3_FORMAL_COHORT_ASSUMPTION_GROUNDED_READY"
    assert all(item["status"] == "BLOCKED_SHARED_M4_GATE" for item in readiness["variant_readiness"].values())
    assert manifest["fixed_contract"]["m3_formal_multi_action_cohort_status"] == "READY_SCENARIO_CONDITIONAL_AUTHORITATIVE_RANKING_GATED"
