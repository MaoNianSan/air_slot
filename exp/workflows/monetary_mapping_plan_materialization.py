...
REGISTRY_FILENAME = "m4_eur_mapping_assumption_grounded_v1.json"
...
def materialize_registry(*, root: Path) -> tuple[Path, MonetaryMappingPlanRegistry]:
    registry = build_plan()
    payload = registry.registry_payload()
    payload["registry_hash"] = registry.digest()
    rendered = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    target = root / "registries" / REGISTRY_FILENAME
    target.parent.mkdir(parents=True, exist_ok=True)
    if target.exists():
        existing = target.read_text(encoding="utf-8")
        if existing != rendered:
            raise RuntimeError("M4_EUR_MAPPING_REGISTRY_EXISTS_WITH_DIFFERENT_CONTENT")
    else:
...
def verify_registry(*, root: Path) -> MonetaryMappingPlanRegistry:
    payload = json.loads((root / "registries" / REGISTRY_FILENAME).read_text(encoding="utf-8"))
    registry = MonetaryMappingPlanRegistry(**payload)
    if registry.registry_hash != registry.digest():
        raise RuntimeError("M4_EUR_MAPPING_REGISTRY_HASH_MISMATCH")
    return registry


def main() -> None:
    parser = argparse.ArgumentParser(description="Materialize the M4 EUR mapping registry")
    parser.add_argument("--root", type=Path, default=Path.cwd())
    parser.add_argument("--verify", action="store_true")
    args = parser.parse_args()
    root = args.root.resolve()
    if args.verify:
        registry = verify_registry(root=root)
    else:
        target, registry = materialize_registry(root=root)
        print(f"wrote {target.relative_to(root)}")
    print(json.dumps({
        "registry_id": registry.registry_id,
        "registry_hash": registry.registry_hash,
        "numeric_freeze_status": registry.numeric_freeze_status.value,
        "frozen_components": [
            rule.component_id for rule in registry.ops_components
            if rule.anchor_status is NumericAnchorStatus.FROZEN_ASSUMPTION_GROUNDED
        ],
        "pending_components": list(registry.pending_anchor_components),
        "tau_comp_selected_minutes": registry.eu261.tau_comp_selected_minutes,
        "tau_comp_sensitivity_minutes": list(registry.eu261.tau_comp_sensitivity_minutes),
        "monetary_system_id": registry.monetary_system_id,
    }, sort_keys=True))
