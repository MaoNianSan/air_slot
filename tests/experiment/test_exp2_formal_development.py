...
def test_formal_development_records_all_variants_and_preserves_guards(tmp_path):
    ...
    assert metrics["variants"]["EXP2B_7COMP"]["execution_status"] == "READY_TYPED_VECTOR_WITH_EXPLICIT_ABSTAIN"
    assert metrics["variants"]["EXP2B_3CHANNEL"]["execution_status"] == "BLOCKED_PASSENGER_CHANNEL_INCOMPLETE"
    assert metrics["variants"]["EXP2B_SCALAR"]["execution_status"] == "BLOCKED_SEVEN_COMPONENT_AGGREGATE_UNRESOLVED"
    ...
    assert lineage["gates"]["M1_TAIL_AWARE_PROPER_SCORES"]["status"] == "BLOCKED_STANDARD_SCALAR_CRPS_AND_VARIOGRAM_UNDEFINED_FOR_EXPLICIT_TAIL_CLASS"
    ...
    assert lineage["gates"]["M2_SEVEN_COMPONENT"]["status"] == "PASS_M2_TYPED_SEVEN_COMPONENT_VECTOR_MATERIALIZED_WITH_ABSTENTION"
    assert lineage["gates"]["M2_SEVEN_COMPONENT"]["seven_component_status_counts"] == {"ABSTAIN": 17250}
