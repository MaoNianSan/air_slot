...
def stable_uniform(seed, episode, scenario, action, response_component):
    ...
    return (int(sha256(key.encode()).hexdigest()[:16], 16) + 0.5) / 2**64
...
def response_value(candidate, *, seed, episode, scenario, component=None):
    ...
    if candidate.response_model == "DETERMINISTIC":
        if "value" not in candidate.response_parameters:
            raise ContractError("ACTION_RESPONSE_PARAMETERS_NOT_FROZEN")
        return float(candidate.response_parameters["value"])
    parameters = candidate.response_parameters
    action = candidate.candidate_action_id
    if candidate.response_model in {"DISCRETE_SCENARIO", "EMPIRICAL"}:
        ...
        if (
            ...
            and candidate.response_model == "DISCRETE_SCENARIO"
        ):
            raise ContractError("ACTION_RESPONSE_PARAMETERS_NOT_FROZEN")
        ...
        u = stable_uniform(seed, episode, scenario, action, candidate.response_model)
        total = 0.0
    ...
    if candidate.response_model != "BERNOULLI_BETA":
        raise ContractError(
            f"RESPONSE_MODEL_NOT_IMPLEMENTED:{candidate.response_model}"
        )
    ...
    if failure == success or any(name not in parameters for name in required):
        raise ContractError("ACTION_RESPONSE_PARAMETERS_NOT_FROZEN")
    probability = (
        1 - float(parameters["failure_probability"])
        if failure
        else float(parameters["success_probability"])
    )
    ...
    if not 0 <= probability <= 1 or not 0 < mean < 1 or concentration <= 0:
        raise ContractError("ACTION_RESPONSE_PARAMETERS_INVALID")
    implemented = (
        stable_uniform(seed, episode, scenario, action, "BERNOULLI") <= probability
    )
    if not implemented:
        return 0.0
