...
def local_hhmm_to_utc(day: date, value: object, timezone_name: str,
                      *, rollover_days: int = 0) -> datetime | None:
    ...
    if minute >= 60 or hour > 24 or (hour == 24 and minute != 0):
        raise ContractError("INVALID_LOCAL_HHMM")
    if hour == 24:
        hour = 0; rollover_days += 1
    try:
        zone = ZoneInfo(timezone_name)
    except ZoneInfoNotFoundError as exc:
        raise ContractError("UNKNOWN_AIRPORT_TIMEZONE") from exc
    local = datetime.combine(day + timedelta(days=rollover_days), datetime.min.time(), zone)
