...
from .models import (DataUsageRule, DatasetCapabilityProfile, RegistryFileIdentity,
                     RegistryManifest, ScientificVariableDefinition, SourcePriorityEntry)
...
_FILES = ("data_usage_rules.yaml", "scientific_variables.yaml",
          "dataset_capabilities.yaml", "source_priority.yaml")
...
def load_registry_bundle(root: Path, *, validate_published: bool = True) -> RegistryBundle:
    try:
        ...
        variables = tuple(ScientificVariableDefinition.model_validate(x)
                          for x in _yaml(paths[1])["variables"])
        profiles = tuple(DatasetCapabilityProfile.model_validate(x)
                         for x in _yaml(paths[2])["profiles"])
        priorities = tuple(SourcePriorityEntry.model_validate(x)
                           for x in _yaml(paths[3])["priorities"])
        ids = tuple(RegistryFileIdentity(
                    path=f"registries/{p.name}",
                    sha256=f"sha256:{sha256(p.read_bytes()).hexdigest()}")
                    for p in paths)
        manifest = RegistryManifest(manifest_version="1.0.0", registries=ids,
        ...
        if validate_published and published_path.is_file():
            published = RegistryManifest.model_validate_json(published_path.read_text(encoding="utf-8"))
            if published != manifest:
                raise RegistryError("published registry manifest mismatch")
        rule_ids = {r.rule_id for r in rules}
        if len(rule_ids) != len(rules): raise RegistryError("duplicate rule_id")
        ...
        for variable in variables:
            ...
            if not dependencies <= declared:
                raise RegistryError(f"unknown or cyclic upstream variable: {sorted(dependencies - declared)}")
            declared.add(variable.scientific_variable)
        return RegistryBundle(data_usage_rules=rules, scientific_variables=variables,
            capability_profiles=profiles, source_priorities=priorities, manifest=manifest)
