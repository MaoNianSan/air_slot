...
from ..membership import validate_observation_membership
...
def core_statistics(
    ...
    match_counts = episodes.get(
        "chain_match_status", pd.Series(dtype="string")
    ).value_counts(dropna=False).to_dict()
    eligibility_counts = {
        column: int(
            episodes.get(column, pd.Series(False, index=episodes.index))
            .fillna(False)
            .astype(bool)
            .sum()
        )
    ...
    partitions = observation_validation.get("partition_counts", {})
    observation_partition_count = (
        sum(int(value) for value in partitions.values())
        if isinstance(partitions, dict) and partitions
        else int(observation_validation.get("partition_count", 0))
    )
    observation_pass_empty = int(
        observation_validation.get("pass_empty_count", 0)
    )
    ...
    return {
        ...
        "observation_rows": int(
            observation_validation.get("observation_rows", 0)
        ),
        ...
        "membership_partition_count": int(
            membership_validation.get("partition_count", 0)
        ),
        ...
        "duplicate_key_counts": {
            "episodes": int(
                episodes.get("chain_episode_id", pd.Series(dtype="string"))
                .duplicated()
                .sum()
            ),
            "events": int(
                events.get("event_id", pd.Series(dtype="string"))
                .duplicated()
                .sum()
            ),
            "observations": int(
                observation_validation.get("duplicate_observation_ids", 0)
            ),
            "membership_ids": int(
                membership_validation.get("duplicate_membership_ids", 0)
            ),
            "membership_relations": int(
                membership_validation.get("duplicate_relations", 0)
            ),
        },
...
def _reference_contract(
    calibration: pd.DataFrame, cfg: dict[str, Any]
) -> dict[str, Any]:
    train_end = pd.Timestamp(cfg["splits"]["train"][1], tz="UTC")
    fit_split_errors = int(calibration["fit_split"].ne("train").sum())
    future_fit = int(
        pd.to_datetime(calibration["fit_end_time"], utc=True).gt(train_end).sum()
    )
    missing_hash = int(
        calibration["source_hash"].fillna("").astype(str).str.len().eq(0).sum()
    )
    return {
        "status": "PASS"
        if not any([fit_split_errors, future_fit, missing_hash])
        else "FAIL",
...
def _leakage_contract(
    ...
    future_evidence = int(
        evidence["future_information_used"].fillna(False).astype(bool).sum()
    )
    unsupported_event_nonnull = int(
        (
            events["support_level"].eq("UNSUPPORTED")
    ...
    identity_errors = int(
        (~episodes.loc[supported, "y_to"].eq(
            episodes.loc[supported, "y_ob"] + episodes.loc[supported, "y_tx"]
        )).sum()
    )
    ...
    return {
        ...
        "target_identity_status": (
            "PASS"
            if supported.any() and not identity_errors
            else "NOT_APPLICABLE"
...
def validate_core(
    ...
    *,
    ...
    components = {
        "tables": _table_contracts(tables, cfg),
        "events": validate_events(tables["events"]),
        "chains": validate_chains(tables["episodes"]),
        "observations": observation_validation,
        "references": _reference_contract(tables["calibration"], cfg),
    ...
    status = (
        "PASS"
        if all(value.get("status") == "PASS" for value in components.values())
        else "FAIL"
    )
    return {
        "status": status,
...
def build_readiness(
    validation: dict[str, Any], episodes: pd.DataFrame
) -> dict[str, Any]:
    ...
    return {
        "status": "ENGINEERING_READY_FOR_ADAPTER"
        if engineering_ready
        else "NOT_READY",
        "engineering_ready": engineering_ready,
        "engineering_adapter_ready": bool(engineering_ready),
        "scientific_chain_ready": bool(
            validation["status"] == "PASS" and scientific_rows > 0
        ),
        ...
        "blockers": []
        if labels_supported
        else ["AOBT_PLUS_AND_SOBT_UNSUPPORTED_FOR_CHAIN_LABELS"],
    }
