...
class HazardBinContract(FrozenModel):
    ...
    finite bin; the maximum supported scalar is included in the terminal
    finite bin and the last class is the strict-above-maximum survival/
    overflow tail.  The induced PMF always sums to one.  A plain categorical
    softmax is never used to imitate this hazard.
    """
...
class M1V2Scenario(FrozenModel):
    ...
    taxi_reference_fallback_level: str | None = None
    taxi_reference_support_state: str | None = None
    positive_tail_used: bool = False
    positive_tail_targets: tuple[str, ...] = ()
    tail_continuation_id: str | None = None
    tail_reference_hash: str | None = None
    ...
    def formal_v2_contract(self):
        ...
        if any(target not in ("D_OB", "D_TX") for target in self.positive_tail_targets):
            raise ValueError("M1_POSITIVE_TAIL_TARGET_INVALID")
        if self.positive_tail_used != bool(self.positive_tail_targets):
            raise ValueError("M1_POSITIVE_TAIL_METADATA_INCONSISTENT")
        if self.positive_tail_used and (
            self.tail_continuation_id is None or self.tail_reference_hash is None
        ):
            raise ValueError("M1_POSITIVE_TAIL_REFERENCE_REQUIRED")
        if not self.positive_tail_used and (
            self.tail_continuation_id is not None or self.tail_reference_hash is not None
        ):
            raise ValueError("M1_POSITIVE_TAIL_REFERENCE_WITHOUT_USE")
        return self
