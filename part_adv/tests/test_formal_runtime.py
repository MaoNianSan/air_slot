...
def test_model_checkpoint_reuse_requires_matching_hashes(tmp_path):
    ...
    assert metadata["formal_target_column"] == "y_movement_raw"
    assert metadata["formal_target_contract_version"] == "Y_MOVEMENT_RAW_V1_20260725"
    ...
    metadata["formal_target_column"] = "y_movement_model"
    metadata_path.write_text(json.dumps(metadata), encoding="utf-8")
