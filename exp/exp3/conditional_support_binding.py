...
def bind(*, root: Path, output_root: Path | None = None) -> dict[str, Path]:
    ...
    output_root = (output_root or root / "artifacts/diagnostics/exp3_conditional_support_binding_v1").resolve()
    paths = {"conditional_support": root / CONDITIONAL, "exp3_manifest": root / EXP3, "action_library": root / LIBRARY, "response_registry": root / RESPONSE, "m2_manifest": root / M2}
    ...
    manifest = {"schema_version": "EXP3_CONDITIONAL_SUPPORT_BINDING_MANIFEST_V1", "status": payload["status"], "artifact": str(artifact_path.resolve()), "artifact_hash": payload["artifact_hash"], "conditional_action_count": 22, "formal_multi_action_status": "BLOCKED_UNCHANGED", "safety": dict(SAFETY)}
