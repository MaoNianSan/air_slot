...
def _finding(check_id: str, status: str, message: str, path: str | None = None):
    return ValidationFinding(
        check_id=check_id, status=status, message=message, path=path
    )
...
def validate_adapters(root: Path) -> list[ValidationFinding]:
    ...
    return [
        _finding(
            "ADAPTER_INTERFACE",
            "PASS",
            str(validate_adapter_interface(adapter, registered)),
        )
...
def validate_pre() -> tuple[list[ValidationFinding], object]:
    ...
    findings = [
        ...
        _finding(
            "FOUNDATION_VALIDATION_SCOPE",
            "PASS",
            "downstream algorithms are outside this fixture-only validation command",
        ),
    ]
...
def run_foundation(command: str, root: Path) -> tuple[ValidationRun, object | None]:
    ...
    if command in {"contracts", "all"}:
        findings.extend(validate_contracts(root))
    if command in {"adapters", "all"}:
        findings.extend(validate_adapters(root))
    if command in {"pre", "all"}:
        pre_findings, fixture = validate_pre()
        findings.extend(pre_findings)
    run_id = content_id(
        {"command": command, "findings": [f.model_dump(mode="json") for f in findings]}
    )
    return (
        ValidationRun(run_id=run_id, command=command, findings=tuple(findings)),
        fixture,
    )
