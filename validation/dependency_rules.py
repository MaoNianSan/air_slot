...
def scan_dependency_boundaries(root: Path) -> list[dict[str, str]]:
    ...
    for path in sorted(root.rglob("*.py")):
        ...
        bad = []
        if rel.startswith("model/") and any(
            m == "exp" or m.startswith("exp.") for m in modules
        ):
            bad.append("MODEL_IMPORTS_EXP")
        if rel.startswith("model/") and any(
            m == "validation" or m.startswith("validation.") for m in modules
        ):
            bad.append("MODEL_IMPORTS_VALIDATION")
        if rel.startswith("model/") and any(
            m == "exp.reporting" or m.startswith("exp.reporting.") for m in modules
        ):
            bad.append("MODEL_IMPORTS_REPORTING")
        if rel.startswith("model/PRE/") and any(
            m.startswith("model.M") for m in modules
        ):
            bad.append("PRE_IMPORTS_DOWNSTREAM")
        if rel.startswith("model/M1/") and any(
            m.startswith("model.PRE.adapters") or m.startswith("model.PRE.realized")
            for m in modules
        ):
            bad.append("M1_IMPORTS_RAW_OR_REALIZED")
        if rel.startswith("model/M2/") and any(
            m.startswith("model.PRE.adapters") or m.startswith("model.PRE.realized")
            for m in modules
        ):
        ...
        if rel.startswith("model/M4/") and any(
            m.startswith("model.PRE.adapters")
            or m.startswith("model.PRE.realized")
            or m == "exp"
            or m.startswith("exp.")
            for m in modules
        ):
            bad.append("M4_IMPORTS_FORBIDDEN_SOURCE")
...
def scan_prohibited_artifacts(root: Path) -> list[dict[str, str]]:
    ...
    forbidden = {
        "LEGACY_GITHUB_IMPORT": "git" + "hub.com/",
        "MACHINE_PATH": "D:" + "\\research\\",
    ...
    for base in roots:
        if not base.exists():
            continue
        for path in sorted(
            p for p in base.rglob("*") if p.suffix in {".py", ".yaml", ".yml"}
        ):
