...
Weather note (v2, after D2-9 alignment): D2-NOAA-ISD now maps to the
current_weather scientific variable (registry canonical_variable=current_weather,
pre_family=current_state, manifest sha256:897bafe4...).  This run injects the
latest legal ISD observation (availability <= cutoff, age <= weather_max_age=60)
for the connection airport into every PRE publish; airports without a legal
observation keep the legal ABSTAIN semantics (NO_LEGAL_RECORD_AT_DECISION_TIME).
"""
...
DATA2 = ROOT / "data2"
OUT = ROOT / "outputs" / "runtime" / "data2_m1_fast_january_wx_v1"
SEED = 20260813
TRAIN_COUNT = 32
...
def main() -> None:
    ...
    weather_index, weather_stats = _weather_index_and_stats(replay_lag)
