...
def _feature_names(cache: M1DevelopmentBaseCache, feature_names: Iterable[str] | None = None) -> tuple[str, ...]:
...
def _train_matrix(cache: M1DevelopmentBaseCache, feature_names: Iterable[str] | None = None):
...
def _contract_key(name: str) -> tuple[str, ...]:
    ...
    row = rows.get(name)
    if row is not None:
        return tuple(str(row[key]) for key in (
            "SOURCE_PRE_VARIABLE", "TRANSFORMATION", "VALIDITY_RULE",
            "HISTORY_SCOPE", "EXPECTED_INFORMATION_ROLE",
        ))
    if name.endswith(".derived_missing_mask"):
        base = name.removesuffix(".derived_missing_mask")
        return (base, "DERIVED_VALIDITY_NEGATION", "SOURCE_LOCAL", "PREVIOUS_NODE_LOCAL", "DERIVED_FEATURE_VALIDITY")
    return (name, "UNSPECIFIED", "UNSPECIFIED", "UNSPECIFIED", "UNSPECIFIED")


def _exact_groups(matrix: np.ndarray, names: tuple[str, ...], splits: np.ndarray) -> list[dict]:
    train = splits == "train"
    ...
    for indices in groups.values():
        ...
        classification = (
            CONTRACT_EXACT_DUPLICATE if len(keys) == 1
            else TRAIN_SUPPORT_CONSTANT if constant_group
            else EMPIRICAL_EXACT_DUPLICATE
        )
        equality = {
            split: bool(all(np.array_equal(
                matrix[splits == split, index], matrix[splits == split, indices[0]]
...
def _complements(matrix: np.ndarray, names: tuple[str, ...], splits: np.ndarray) -> list[dict]:
    output = []
    train = splits == "train"
    binary = [
        column for column in range(matrix.shape[1])
    ...
    for offset, left in enumerate(binary):
        for right in binary[offset + 1:]:
            ...
            if valid.any() and np.array_equal(matrix[valid, left] + matrix[valid, right], np.ones(int(valid.sum()))):
                output.append({"classification": DETERMINISTIC_COMPLEMENT, "left": names[left], "right": names[right]})
    return output


def _near_linear(matrix: np.ndarray, names: tuple[str, ...], splits: np.ndarray) -> list[dict]:
    output = []
    train = splits == "train"
    for left in range(matrix.shape[1]):
        ...
        for right in range(left + 1, matrix.shape[1]):
            ...
            if np.isfinite(correlation) and abs(correlation) >= 0.999:
                output.append({"classification": NEAR_LINEAR_REPORT_ONLY, "left": names[left], "right": names[right], "pearson_correlation": correlation, "automatic_removal": False})
    return output
...
def _contract_affine_redundancy(names: tuple[str, ...]) -> list[dict]:
    if _SCHEDULE_DELTA not in names or _SCHEDULE_DELTA_MASK not in names:
        return []
    return [{"classification": "CONTRACT_AFFINE_REDUNDANCY", "features": [_SCHEDULE_DELTA, _SCHEDULE_DELTA_MASK], "reason": "FIXED_FIVE_MINUTE_GRID_SCHEDULE_COUNTDOWN_DELTA"}]
...
def _repeated_weather_masks(names: tuple[str, ...]) -> dict:
    ...
    for kind in ("stale", "fallback"):
        ...
        field_names = tuple(f"weather.{field}.{kind}_mask" for field in V2_WEATHER_FIELDS)
        output[kind] = {"object_level_feature": object_name, "field_level_features_present": [name for name in field_names if name in name_set], "object_level_present": object_name in name_set, "classification": "COLLAPSED_OBJECT_LEVEL_MASK", "recommendation": "KEEP_OBJECT_LEVEL_ONLY"}
    return output


def redundancy_audit(cache: M1DevelopmentBaseCache, feature_names: Iterable[str] | None = None) -> dict:
    matrix, splits, names = _train_matrix(cache, feature_names)
    groups = _exact_groups(matrix, names, splits)
    contract_groups = [g for g in groups if g["classification"] == CONTRACT_EXACT_DUPLICATE]
    empirical_groups = [g for g in groups if g["classification"] == EMPIRICAL_EXACT_DUPLICATE]
    support_groups = [g for g in groups if g["classification"] == TRAIN_SUPPORT_CONSTANT]
    train = splits == "train"
    train_support_constants = [names[index] for index in range(matrix.shape[1]) if np.nanstd(matrix[train, index]) <= 1e-12]
    structural_constants = [name for name in train_support_constants if name.startswith("state.") and name.endswith("_mask")]
    return {
...
__all__ = [
    "CONTRACT_EXACT_DUPLICATE", "EMPIRICAL_EXACT_DUPLICATE", "TRAIN_SUPPORT_CONSTANT",
    "CONTRACT_STRUCTURAL_CONSTANT", "DETERMINISTIC_COMPLEMENT", "NEAR_LINEAR_REPORT_ONLY",
    "redundancy_audit",
]
