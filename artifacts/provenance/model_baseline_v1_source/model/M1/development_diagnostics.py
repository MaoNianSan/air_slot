...
def _finite_discrete_crps(
    values: list[float], probabilities: list[float], target: float
) -> float:
    first = sum(
...
def _weighted_quantile(
    values: list[float], probabilities: list[float], level: float
) -> float:
    cumulative = 0.0
    for value, probability in zip(values, probabilities, strict=True):
...
def _evaluate_distributions(
    ...
    *,
    ...
    calibration_pairs: dict[str, list[tuple[float, float]]] = {key: [] for key in brier}
    ...
    for start in range(0, len(examples), batch_size):
        ...
        successor = {
            name: {
                ...
                "quantiles": distributions[name]["positive_quantiles_minutes"]
                .detach()
                .cpu(),
            }
            for name in ("D_OB", "D_TX")
        ...
        for index, example in enumerate(batch):
            ...
            if example.active.get(M1_V2_HAZARD_COORDINATE) and target is not None:
                ...
                calibration_pairs["T_IB_GT_30"].append(
                    (probability_gt_30, outcome_gt_30)
                )
                if target < hazard.max_finite_minutes and normalized:
                    ...
                    coverage[M1_V2_HAZARD_COORDINATE].append(
                        float(lower <= target <= upper)
                    )
            ...
            for name in ("D_OB", "D_TX"):
                ...
                quantiles = [
                    float(value) for value in successor[name]["quantiles"][index]
                ]
                median_index = min(
                ...
                if target > 0:
                    coverage[name].append(
                        float(quantiles[0] <= target <= quantiles[-1])
                    )
...
def evaluate_fast_predictor(
    predictor, examples, *, batch_size: int
) -> dict[str, object]:
    return _evaluate_distributions(
        examples,
        predictor.contracts,
...
def paired_state_difference(reference: dict, comparison: dict) -> float | None:
    ...
    differences = [
        abs(
            reference_values[row["decision_node_id"]]
