...
class Data1Adapter:
    _families = ("opensky_state_vectors", "opensky_flightlist", "iem_metar", "eurostat", "ourairports")

    def describe(self) -> AdapterDescription:
        return AdapterDescription(dataset_instance_id="data1_2019", source_families=self._families)

    def capabilities(self) -> dict[str, str]:
        return {"qnh_mslp": "QNH_NOT_MSLP", "schedule": "UNSUPPORTED",
                "aircraft_metadata_2019": "UNSUPPORTED", "passenger_reference": "EMPIRICAL_REFERENCE"}
    ...
    def validate_source(self, request: SourceValidationRequest) -> SourceValidationReport:
        ...
        return SourceValidationReport(dataset_instance_id="data1_2019", source_family=request.source_family,
            status="DECLARED" if supported else "UNSUPPORTED",
            reason_code=None if supported else "SOURCE_FAMILY_NOT_DECLARED")

    def iter_canonical(self, request: RawReadRequest, *, replay_lag_minutes: int | None = None):
        ...
        converters = {"iem_metar": lambda row: canonicalize_metar_row(row, replay_lag_minutes=replay_lag_minutes),
                      "opensky_flightlist": canonicalize_flightlist_row,
                      "opensky_state_vectors": lambda row: canonicalize_state_vector_row(row, replay_lag_minutes=replay_lag_minutes),
                      "ourairports": canonicalize_airport_row,
                      "eurostat": lambda row: _eurostat_converter(row["payload"])}
        if request.source_family not in converters: raise ContractError("CANONICALIZER_NOT_IMPLEMENTED")
