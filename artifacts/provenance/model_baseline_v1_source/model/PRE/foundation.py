...
def build_pre_state(request: PREBuildRequest) -> PREBuildResult:
    ...
    targets = (
        TargetSupportState(
            ...
            active=True,
            support_state=SupportState.SUPPORTED,
            target_definition_id="R_IB_V1",
            dataset_ceiling=EvidenceClass.DIRECT,
            formal_input_support=EvidenceClass.DIRECT,
            realized_outcome_support=EvidenceClass.DERIVED,
        ),
        TargetSupportState(
            target_name="DELTA_OB",
            active=r_ob_supported,
            support_state=(
                SupportState.SUPPORTED if r_ob_supported else SupportState.ABSTAIN
            ),
            target_definition_id="DELTA_OB_V1",
            dataset_ceiling=(
                EvidenceClass.DIRECT if r_ob_supported else EvidenceClass.UNSUPPORTED
            ),
            formal_input_support=(
                EvidenceClass.DIRECT if r_ob_supported else EvidenceClass.UNSUPPORTED
            ),
            realized_outcome_support=(
                EvidenceClass.DIRECT if r_ob_supported else EvidenceClass.DERIVED
            ),
            abstention_reason=(
                None if r_ob_supported else "TARGET_SEMANTICS_UNSUPPORTED"
            ),
        ),
        TargetSupportState(
            target_name="T_TX",
            active=True,
            support_state=SupportState.SUPPORTED,
            target_definition_id="T_TX_V1",
            dataset_ceiling=EvidenceClass.DERIVED,
            formal_input_support=EvidenceClass.DERIVED,
            realized_outcome_support=EvidenceClass.DERIVED,
        ),
    )
    motion = _value(0)
    ...
    ledger = (
        EvidenceLedgerEntry(
            ...
            scientific_object="predecessor_motion",
            source_name="fixture_motion",
            source_record_id="motion-1",
            event_time=request.information_cutoff,
            availability_time=request.information_cutoff,
        ...
        EvidenceLedgerEntry(
            decision_node_id=node.decision_node_id,
            scientific_object="current_weather",
            source_name="fixture_weather",
    ...
    lineage = (
        VariableLineageEntry(
            decision_node_id=node.decision_node_id,
            scientific_variable="predecessor_motion",
            supported_value=motion,
            canonical_variable="predecessor_motion",
            rule_id="D1-OPENSKY-STATE",
            source_name="fixture_motion",
            source_record_id="motion-1",
            event_time=request.information_cutoff,
            availability_time=request.information_cutoff,
        ...
        VariableLineageEntry(
            ...
            supported_value=weather,
            canonical_variable="current_weather",
            rule_id="D1-METAR",
            source_name="fixture_weather",
            source_record_id="weather-1",
            event_time=request.information_cutoff,
    ...
    return PREBuildResult(
        pre_state=PREState(
            decision_node=node,
            predecessor_state={"motion": motion},
            current_state={"weather": weather},
            successor_state={},
            evidence_ledger=ledger,
            variable_lineage=lineage,
            reference_state=ReferenceState(entries={}),
