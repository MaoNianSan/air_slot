...
def source_files(
    request: RawReadRequest, definition: SourceAdapterDefinition
) -> list[Path]:
    files: set[Path] = set()
    for template in definition.relative_globs:
        pattern = template.format(
            year=request.year or "*",
            month=f"{request.month:02d}" if request.month else "*",
...
def iter_csv_rows(
    ...
    for path in source_files(request, definition):
        ...
        fingerprint = (
            f"sha256:{sha256(prefix + str(path.stat().st_size).encode()).hexdigest()}"
        )
        if path.suffix.lower() == ".json":
            payload = json.loads(path.read_text(encoding="utf-8"))
            missing_columns = set(definition.required_columns) - set(payload)
            if missing_columns:
                raise ContractError(f"RAW_SCHEMA_MISMATCH:{sorted(missing_columns)}")
            yield RawRow(
                values={"payload": payload},
                source_path=path.relative_to(request.raw_root).as_posix(),
                source_fingerprint=fingerprint,
                row_number=1,
            )
            emitted += 1
            if request.max_rows and emitted >= request.max_rows:
                return
            continue
        ...
        with opener(
            ...
            missing_columns = set(definition.required_columns) - set(
            ...
            for row_number, row in enumerate(reader, start=2):
                values = {
                    key: row.get(key, "")
                    for key in (definition.projected_columns or tuple(row))
                }
                yield RawRow(
                    values=values,
                    source_path=path.relative_to(request.raw_root).as_posix(),
...
def _iter_tar_rows(
    ...
    with tarfile.open(path, "r") as archive:
        ...
        if not members:
            raise ContractError("ARCHIVE_HAS_NO_REGISTERED_CSV_MEMBER")
        ...
        for member in sorted(members, key=lambda item: item.name):
            ...
            with io.TextIOWrapper(
                ...
                if missing_columns:
                    raise ContractError(
                        f"RAW_SCHEMA_MISMATCH:{sorted(missing_columns)}"
                    )
                for row_number, row in enumerate(reader, start=2):
                    values = {
                        key: row.get(key, "")
