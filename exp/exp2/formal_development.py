...
def _validate_fixed_contract(root: Path, inputs: dict[str, tuple[Path, dict[str, Any]]]) -> dict[str, Any]:
    ...
    for payload in (binding, exp1, cohort):
...
def _gates(inputs: dict[str, tuple[Path, dict[str, Any]]], fixed: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    return {
        "M1_COHORT_BINDING": {
            "status": "BLOCKED_M1_INPUT_COHORT_NODE_IDENTITY_MISMATCH",
            "reason": "FROZEN_M1_CACHE_HAS_NO_NODE_ID_OVERLAP_WITH_FROZEN_EXP2_DEVELOPMENT_COHORT",
