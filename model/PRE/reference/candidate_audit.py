...
def _missing(value) -> bool:
    return value is None or str(value).strip().upper() in {"", "NA", "N/A", "NAN", "NONE", "NULL"}
...
def _utc(day: date, hhmm, zone: str):
    if _missing(hhmm): return None
    text = str(hhmm).strip().split(".")[0].zfill(4)
    try: hour, minute = int(text[:-2]), int(text[-2:])
    except ValueError: return None
...
def _summary(values):
    return {"n": len(values), "mean": sum(values)/len(values) if values else None,
            **{f"p{int(p*100):02d}": _quantile(values, p)
               for p in (.05, .20, .50, .80, .95)},
            "min": min(values) if values else None, "max": max(values) if values else None}


def _hist_summary(hist):
    count = sum(hist.values())
    if not count: return {"n":0,"mean":None,"p05":None,"p20":None,"p50":None,
                          "p80":None,"p95":None,"min":None,"max":None}
    ordered = sorted(hist)
    def quantile(p):
        index=p*(count-1); lo,hi=math.floor(index),math.ceil(index)
        def at(position):
            cumulative=0
    ...
    return {"n":count,"mean":sum(value*n for value,n in hist.items())/count,
            **{f"p{int(p*100):02d}":quantile(p) for p in (.05,.20,.50,.80,.95)},
            "min":ordered[0],"max":ordered[-1]}


def _sql_values(db, kind, part):
    return (row[0] for row in db.execute(
        "SELECT value FROM refs WHERE kind=? AND part=? ORDER BY value", (kind, part)))
...
def _sql_summary(db, kind, part, *, eligibility_max=None):
    ...
    count, mean, minimum, maximum = db.execute(
        "SELECT count(*),avg(value),min(value),max(value) FROM refs WHERE kind=? AND part=?" + extra,
        args).fetchone()
    ...
    for p in (.05, .20, .50, .80, .95):
        if not count:
            ...
            values = [row[0] for row in db.execute(
                "SELECT value FROM refs WHERE kind=? AND part=?" + extra +
                " ORDER BY value LIMIT 2 OFFSET ?", args + (lo,))]
            value = values[0] if lo == hi else values[0] + (values[1] - values[0]) * (index - lo)
...
def _group_refs(db, kind, keys, percentile, min_n, *, eligibility_max=None):
    ...
    ordered = db.execute(f"SELECT {columns},value FROM refs WHERE kind=? AND part='train'{extra} ORDER BY {columns},value", args)
    refs, cell_counts, current, values = {}, {}, None, []
    def flush():
        if current is not None:
            cell_counts[current] = len(values)
            if len(values) >= min_n: refs[current] = _quantile(values, percentile)
    for row in ordered:
        key = (row[0],) if len(keys) == 1 else (row[0], row[1])
        if current is not None and key != current:
            flush(); values = []
        current = key; values.append(row[-1])
    flush()
    ...
    for row in rows:
        ...
        for name, keys, refs in levels:
            if tuple(row[key] for key in keys) in refs:
                selected = name; break
        counts[selected] += 1
    return {name: {"n": count, "fraction": count/total if total else 0}
...
def main():
    ...
    source_paths = [zones_path] + [_zip(month) for months in MONTHS.values() for month in months]
    before = {str(path.relative_to(ROOT)): (path.stat().st_size, path.stat().st_mtime_ns)
    ...
    with tempfile.TemporaryDirectory(prefix="m2_audit_") as temporary:
        ...
        db.execute("CREATE TABLE flights(part TEXT, carrier TEXT, tail TEXT, origin TEXT, dest TEXT, sdep REAL, sarr REAL, adep REAL, aarr REAL, taxi REAL)")
        db.execute("CREATE TABLE refs(kind TEXT, part TEXT, k1 TEXT, k2 TEXT, value REAL, eligibility REAL)")
        counts = defaultdict(int)
        for part, months in MONTHS.items():
            for month in months:
                ...
                for row in _iter_rows(month):
                    ...
                    if not tail or origin not in zones or dest not in zones: continue
                    try: day = date.fromisoformat(row["FlightDate"][:10])
                    ...
                    if len(batch) >= 20_000:
                        db.executemany("INSERT INTO flights VALUES (?,?,?,?,?,?,?,?,?,?)", batch); batch.clear()
                if batch: db.executemany("INSERT INTO flights VALUES (?,?,?,?,?,?,?,?,?,?)", batch)
                db.commit()
        db.execute("CREATE INDEX ix_flights ON flights(part,tail,sdep)")
        db.execute("INSERT INTO refs SELECT 'taxi',part,origin,carrier,taxi,NULL FROM flights WHERE taxi IS NOT NULL AND taxi>=0")
        db.execute("""INSERT INTO refs
            WITH ordered AS (
        ...
        for part in MONTHS:
            ...
            def flush_tail():
                if not departures: return
                for horizon in HORIZONS:
                    right = 0
                    for left, departure in enumerate(departures):
                        right = max(right, left + 1)
                        while right < len(departures) and departures[right] <= departure + horizon:
                            right += 1
            ...
            for tail, departure in db.execute(
                ...
                if current_tail is not None and tail != current_tail:
                    flush_tail(); departures = []
                current_tail = tail; departures.append(departure)
            flush_tail()
            horizon_by_part[part] = {str(h): {**_hist_summary(values),
                "positive_fraction": sum(n for value,n in values.items() if value > 0) / sum(values.values())}
                for h, values in horizon_values.items()}
            counts[f"{part}_flights"] = sum(next(iter(horizon_values.values())).values())
        ...
        airport_turn, airport_turn_n = _group_refs(db, "turn", ("airport",), .5, 100)
        airport_carrier_turn, airport_carrier_turn_n = _group_refs(db, "turn", ("airport","carrier"), .5, 100)
        global_taxi_median = {(): taxi_train_summary["p50"]}
        airport_taxi_median, airport_taxi_n = _group_refs(db, "taxi", ("origin",), .5, 100)
        global_taxi_p20 = {(): taxi_train_summary["p20"]}
        airport_taxi_p20, _ = _group_refs(db, "taxi", ("origin",), .2, 100)
        airport_carrier_taxi_p20, airport_carrier_taxi_n = _group_refs(
            db, "taxi", ("origin","carrier"), .2, 100)
        def dev_rows(kind, names, eligibility_max=None):
            columns = "k1,value" if len(names)==1 else "k1,k2,value"
            extra = "" if eligibility_max is None else " AND eligibility>=0 AND eligibility<=?"
            args = (kind,) if eligibility_max is None else (kind,eligibility_max)
            for row in db.execute(f"SELECT {columns} FROM refs WHERE kind=? AND part='development'{extra}", args):
        ...
        for window in HORIZONS:
            ...
            airport_ref, airport_n = _group_refs(db,"turn",("airport",),.5,100,
                                                 eligibility_max=window)
            airport_carrier_ref, airport_carrier_n = _group_refs(
                db,"turn",("airport","carrier"),.5,100,eligibility_max=window)
            turn_window_candidates[str(window)] = {
                ...
                "airport_carrier_cell_count_distribution":_summary(list(airport_carrier_n.values())),
                "development_fallback_coverage":_coverage(
                    dev_rows("turn",("airport","carrier"),window),
                    (("AIRPORT_CARRIER",("airport","carrier"),airport_carrier_ref),
                     ("AIRPORT",("airport",),airport_ref),("GLOBAL",(),global_ref)))}

        metrics = {
            "status": "DEVELOPMENT_CANDIDATES_ONLY", "production_artifact": False,
            "partitions": {"fit": "2019-01--2019-06", "diagnostic": "2019-08--2019-09",
                           "excluded": ["2019-07", "2019-10--2019-12"]},
            "sources": list(before), "raw_read_only": before == {
                str(path.relative_to(ROOT)): (path.stat().st_size, path.stat().st_mtime_ns)
                for path in source_paths},
            "counts": {key: value for key, value in counts.items() if not key.endswith("horizon_metrics")},
            "turnaround": {
                "train_distribution_minutes": turn_train_summary,
                "development_distribution_minutes": _sql_summary(db,"turn","development"),
                "scheduled_connection_window_candidates":turn_window_candidates,
                "candidates": {
                    "T1_GLOBAL_MEDIAN": {"value_minutes": global_turn[()],
                        "development_coverage": _coverage(dev_rows("turn",("airport",)), (("GLOBAL",(),global_turn),))},
                    "T2_AIRPORT_MEDIAN_N100_GLOBAL": {"eligible_airport_cells": len(airport_turn),
                        "cell_count_distribution": _summary(list(airport_turn_n.values())),
                        "development_coverage": _coverage(dev_rows("turn",("airport",)), (("AIRPORT",("airport",),airport_turn),("GLOBAL",(),global_turn)))},
                    "T3_AIRPORT_CARRIER_MEDIAN_N100_AIRPORT_GLOBAL": {
                        "eligible_airport_carrier_cells": len(airport_carrier_turn),
                        "cell_count_distribution": _summary(list(airport_carrier_turn_n.values())),
                        "development_coverage": _coverage(dev_rows("turn",("airport","carrier")), (("AIRPORT_CARRIER",("airport","carrier"),airport_carrier_turn),("AIRPORT",("airport",),airport_turn),("GLOBAL",(),global_turn)))},
                }},
            "taxi": {
                "train_distribution_minutes": taxi_train_summary,
                "development_distribution_minutes": _sql_summary(db,"taxi","development"),
                "candidates": {
                    "X1_GLOBAL_MEDIAN": {"value_minutes": global_taxi_median[()],
                        "development_coverage": _coverage(dev_rows("taxi",("origin",)), (("GLOBAL",(),global_taxi_median),))},
                    "X2_AIRPORT_MEDIAN_N100_GLOBAL": {"eligible_airport_cells": len(airport_taxi_median),
                        "cell_count_distribution": _summary(list(airport_taxi_n.values())),
                        "development_coverage": _coverage(dev_rows("taxi",("origin",)), (("AIRPORT",("origin",),airport_taxi_median),("GLOBAL",(),global_taxi_median)))},
                    "X3_AIRPORT_CARRIER_P20_N100_AIRPORT_GLOBAL": {
                        ...
                        "cell_count_distribution": _summary(list(airport_carrier_taxi_n.values())),
                        "development_coverage": _coverage(dev_rows("taxi",("origin","carrier")), (("AIRPORT_CARRIER",("origin","carrier"),airport_carrier_taxi_p20),("AIRPORT",("origin",),airport_taxi_p20),("GLOBAL",(),global_taxi_p20)))},
                }},
        ...
        metrics["candidate_metrics_sha256"] = f"sha256:{sha256(payload.encode()).hexdigest()}"
