...
class OrderedEventGRU(nn.Module):
    ...
    def __init__(
        self, input_size: int, hidden_size: int, bins: dict[str, TargetBinContract]
    ):
        super().__init__()
        ...
        self.delta_ob_embedding = nn.Embedding(
            bins["DELTA_OB"].class_count, hidden_size
        )
    ...
    def conditioned_logits(
        ...
        ib = torch.as_tensor(ib_index, dtype=torch.long, device=history.device).reshape(
            -1
        )
        ...
        if delta_ob_index is None:
            ...
            delta_ob = torch.as_tensor(
                delta_ob_index, dtype=torch.long, device=history.device
            ).reshape(-1)
    ...
    def forward(
        ...
        ibc = self._condition(
            ib,
            self.ib_embedding,
            None if teacher is None else teacher.get("R_IB"),
            active.get("R_IB"),
        )
...
class FastRepresentationEncoder(nn.Module):
    ...
    recurrent hidden dimension (``hidden_size``). The active model uses H=16;
    H=8 is the predefined lower-capacity sensitivity setting. ``r_fast``
...
class StaticRepresentationEncoder(nn.Module):
    ...
    dimension, restricted to the frozen H=16 primary or H=8 lower-capacity
    sensitivity setting.
    """
...
class M1V2GRU(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.current_observation_encoder = (
            None if self.history_encoder_enabled else nn.Linear(input_size, hidden_size)
        )
        ...
        self.d_ob_quantile_head = nn.Linear(
            self.state_width + hidden_size, d_ob.quantile_count
        )
        ...
        self.d_tx_quantile_head = nn.Linear(
            self.state_width + hidden_size * 2, d_tx.quantile_count
        )
    ...
    def state_representation(
        ...
        if self.fast_encoder is not None:
            blocks.append(
                self.fast_encoder(fast_features)
    ...
    def d_tx_heads(
        ...
        ibc = self.ib_embedding(self._as_index(ib_index, state.device, state.shape[0]))
        d_obc = self.d_ob_embedding(
            self._as_index(d_ob_index, state.device, state.shape[0])
        )
