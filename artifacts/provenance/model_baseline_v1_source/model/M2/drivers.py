...
from model.common.native_formulas import p_itinerary_native, p_service_native, p_time_native
...
COMPONENT_INPUT_CONTRACTS = {
    ...
    for item in (
        ...
        ComponentInputContract(
            ...
            critical_inputs=("d_to_minutes", "expected_passengers_per_flight", "connection_share_reference", "itinerary_buffer_reference"),
            irrelevant_inputs=tuple(
                name
                for name in _CONTEXT_FIELDS
                if name not in {"expected_passengers_per_flight", "connection_share_reference", "itinerary_buffer_reference"}
            ),
        ),
        ComponentInputContract(
            component_id="P_service",
            critical_inputs=("d_to_minutes", "expected_passengers_per_flight", "service_policy_reference"),
            irrelevant_inputs=tuple(
                name
                for name in _CONTEXT_FIELDS
                if name not in {"expected_passengers_per_flight", "service_policy_reference"}
            ),
        ),
        ComponentInputContract(
...
def _connection_share_value(reference: ScientificContextValue) -> float | None:
    if reference.value is None or reference.support_state is SupportState.ABSTAIN:
        return None
    try:
...
def native_quantities(
    ...
    rows = (
        ...
        publish(
            "P_time",
            lambda: p_time_native(float(expected_pax().value), d_to),
            "passenger_minutes",
        ...
        publish(
            "P_itinerary",
            lambda: p_itinerary_native(
                float(expected_pax().value),
                float(_connection_share_value(ctx("connection_share_reference"))),
                d_to,
                float(ctx("itinerary_buffer_reference").value),
            ),
            "expected_disrupted_connecting_passenger_exposure",
            "expected_passengers_x_connecting_share_x_itinerary_threshold",
            (("d_to_minutes", d_to, d_to_support),),
            ("expected_passengers_per_flight", "connection_share_reference", "itinerary_buffer_reference"),
            EvidenceClass.DOMAIN_PROXY,
            SourceType.HYBRID,
        ),
        publish(
            "P_service",
            lambda: p_service_native(float(expected_pax().value), d_to, float(ctx("service_policy_reference").value)),
            "expected_long_delay_passenger_service_exposure",
            "expected_passengers_x_service_threshold",
            (("d_to_minutes", d_to, d_to_support),),
            ("expected_passengers_per_flight", "service_policy_reference"),
            EvidenceClass.DOMAIN_PROXY,
            SourceType.HYBRID,
        ),
        publish(
