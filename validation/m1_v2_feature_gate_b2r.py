...
B1R_CACHE_HASH = (
    "sha256:57c668b687bf08b9aa0ebfbd60c0b6792d970a38e39e5a5d104294aa44f3153e"
)
B1R_SCHEMA_HASH = (
    "sha256:0fa54b2b5d2db31ed15b10c6a4f3eba8dd1b3e3e82300cc70f6417266057930b"
)
...
def _load_b1r() -> tuple[M1DevelopmentBaseCache, dict, dict, dict]:
    ...
    if (
        manifest["cache_hash"] != B1R_CACHE_HASH
        or report["cache"]["candidate_cache_hash"] != B1R_CACHE_HASH
    ):
        raise ValueError("M1_B2R_B1R_CACHE_HASH_MISMATCH")
    if (
        schema["schema_hash"] != B1R_SCHEMA_HASH
        or report["feature_schema"]["schema_hash"] != B1R_SCHEMA_HASH
    ):
...
def _candidate_schema(b1r_schema: dict) -> dict:
    ...
    payload["ordered_dynamic_features"] = [
        name for name in b1r_schema["ordered_dynamic_features"] if name not in removed
    ]
    ...
    payload["total_feature_count"] = (
        payload["dynamic_feature_count"] + payload["static_feature_count"]
    )
    payload["features"] = [
        row for row in b1r_schema["features"] if row["FEATURE"] not in removed
    ]
    payload["groups"] = {
    ...
    payload["removed"]["all_exact_removed_feature_names"] = list(
        dict.fromkeys(
            payload["removed"].get("all_exact_removed_feature_names", [])
            + list(REMOVED_SCHEDULE_DELTA)
        )
    )
    payload["applied_decisions"] = {**b1r_schema["applied_decisions"], **DECISIONS}
    payload["source_b1r_schema_hash"] = b1r_schema["schema_hash"]
    payload["source_b1r_candidate_feature_count"] = len(
        b1r_schema["ordered_dynamic_features"]
    )
...
def _identity_equal(left, right) -> dict[str, bool]:
    return {
        ...
        "episode_offsets": left.store.episode_offsets.equal(
            right.store.episode_offsets
        ),
        "sample_episode_indices": left.store.sample_episode_indices.equal(
            right.store.sample_episode_indices
        ),
        "sample_start_offsets": left.store.sample_start_offsets.equal(
            right.store.sample_start_offsets
        ),
        "sample_end_offsets": left.store.sample_end_offsets.equal(
            right.store.sample_end_offsets
        ),
        "sample_episode_ids": left.store.sample_episode_ids
        == right.store.sample_episode_ids,
        "sample_decision_node_ids": left.store.sample_decision_node_ids
        == right.store.sample_decision_node_ids,
        "sample_episode_dates": left.store.sample_episode_dates
        == right.store.sample_episode_dates,
        "sample_splits": left.store.sample_splits == right.store.sample_splits,
        "static_context_lineages": left.store.static_context_lineages
...
def _static_artifact(cache: M1DevelopmentBaseCache) -> dict:
    ...
    a2_result = _read_json(
        ...
        / "diagnostics"
        / "m1_v2_data_gate_a2"
        / "AIR_SLOT_M1_V2_DATA_GATE_A2.json"
    )
...
def run(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    keep_indices = [
        ...
        if name not in REMOVED_SCHEDULE_DELTA
    ]
    if tuple(name for name in old_names if name not in REMOVED_SCHEDULE_DELTA) != tuple(
        FEATURE_NAMES_V2
    ):
        raise ValueError("M1_B2R_MODEL_SCHEMA_ORDER_MISMATCH")
    candidate_store = replace(
        b1r_cache.store,
        values_flat=b1r_cache.store.values_flat[:, keep_indices].contiguous(),
    )
    ...
    static_unchanged = candidate.static_normalization.model_dump(
        mode="json"
    ) == b1r_cache.static_normalization.model_dump(mode="json")
    ...
    gate_checks = {
        ...
        "a2_provenance": b1r_manifest["provenance"]["a2_cache_hash"]
        == A2_EXPECTED_CACHE_HASH,
        "static_normalization_unchanged": static_unchanged
        and candidate.static_normalization.episode_count == 128,
    ...
    status = (
        ...
        else "FEATURE_GATE_B2R_CONTRACT_FAILURE"
    )
    ...
    report = {
        ...
        "b2_failure_diagnosis": {
            "old_exact_duplicate_gate": "TRAIN_CURRENT_ROWS_ANY_EXACT_DUPLICATE_BLOCKED",
            "new_semantic_duplicate_gate": "CONTRACT_EXACT_DUPLICATE_AND_CONTRACT_STRUCTURAL_CONSTANT_ONLY",
        },
        "decisions": DECISIONS,
        "feature_counts": {
            "dynamic": len(FEATURE_NAMES_V2),
        ...
        "feature_schema": {
            ...
            "b1r_schema_hash": b1r_schema["schema_hash"],
            "b2r_candidate_schema_hash": schema["schema_hash"],
            "schema_artifact": "M1_V2_FEATURE_SCHEMA_CANDIDATE_B2R.json",
        },
        "schedule_delta": {
            "fixed_grid_proof": "VALID_PREVIOUS_NODE_TRANSITION_ON_FIVE_MINUTE_GRID_HAS_FIXED_COUNTDOWN_DELTA",
            "numeric_removed": list(REMOVED_SCHEDULE_DELTA)[0],
            "mask_removed": list(REMOVED_SCHEDULE_DELTA)[1],
            "remaining_current_position": "schedule.signed_minutes_to_crs_departure",
            "present_in_candidate": any(
                name in FEATURE_NAMES_V2 for name in REMOVED_SCHEDULE_DELTA
            ),
        },
        "redundancy": {
            ...
            "deterministic_complements": redundancy["deterministic_complements"],
        ...
        "static": {
            "normalization_unchanged": static_unchanged,
            "fitting_unit": "UNIQUE_TRAIN_EPISODE_STATIC_CONTEXTS",
            "episode_count": candidate.static_normalization.episode_count,
            "turnaround": static_values["turnaround_reference_minutes"].model_dump(
                mode="json"
            ),
            "taxi": static_values["taxi_reference_minutes"].model_dump(mode="json"),
            "partial_missing_preserved": partial_cases == 4,
            "partial_missing_cases": partial_cases,
        },
        "missing_invariants": {
        ...
        "exp1b": history,
        "labels": {
            "unchanged": labels_unchanged,
            "overflow": b1r_report["target"]["overflow"],
            "TARGET_SUPPORT_REVIEW_REQUIRED": "YES",
        },
        "cache": {
            "b1r_candidate_cache": b1r_manifest["cache_hash"],
            "candidate_cache": saved_manifest["cache_hash"],
            "candidate_cache_key": candidate_key,
            "schema_version": CACHE_SCHEMA_VERSION,
            "roundtrip": roundtrip,
            "a2_identity_labels_active_lineage_ids_unchanged": identity,
        },
        "profiles": {
            "train": profiles["profiles"]["train"],
            "shift_report_only": shift_diagnostics(
                profiles["profiles"],
                list(FEATURE_NAMES_V2) + list(STATIC_FEATURE_NAMES),
            ),
        },
        ...
        json.dumps(
            {
                ...
                "candidate_schema_hash": report["feature_schema"][
                    "b2r_candidate_schema_hash"
                ],
                "candidate_cache_hash": report["cache"]["candidate_cache"],
