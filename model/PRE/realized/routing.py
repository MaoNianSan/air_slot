...
_DEGRADED_DETECTOR_FLAGS = {"STATE_MACHINE_CROSSCHECK_MISMATCH", "ONGROUND_MISSING_INFERRED"}
...
def _in_window(event: OperationalEventRecord, *, aircraft_id: str | None,
...
def route_predecessor_arrival(*, flight_id: str, aircraft_id: str | None,
                              window_start: datetime, window_end: datetime,
                              events: tuple[OperationalEventRecord, ...],
                              proxy_event: OperationalEventRecord | None = None) -> RoutedArrival:
    for event_type in _ARRIVAL_PRIORITY:
        matches = [event for event in events
                   if event.event_type == f"TRAJECTORY_{event_type}"
                   and _in_window(event, aircraft_id=aircraft_id,
                                  window_start=window_start, window_end=window_end)]
        if matches:
            chosen = _latest(matches)
            return RoutedArrival(flight_id=flight_id, arrival_utc=chosen.event_time,
                source=event_type, support_state=_detector_support(chosen),
                reason_code=None,
                parent_record_ids=(chosen.canonical_record_id,),
                quality_flags=("ARRIVAL_SOURCE_TRAJECTORY",) + chosen.quality_flags)
    if proxy_event is not None and proxy_event.event_type == "ARCHIVE_FLIGHT_INTERVAL_PROXY" \
            and proxy_event.event_time_upper is not None:
        return RoutedArrival(flight_id=flight_id, arrival_utc=proxy_event.event_time_upper,
            source="FLIGHTLIST_PROXY", support_state=SupportState.DEGRADED,
    ...
    return RoutedArrival(flight_id=flight_id, arrival_utc=None, source="NONE",
        support_state=SupportState.ABSTAIN,
        reason_code="NO_REALIZED_ARRIVAL_EVIDENCE", parent_record_ids=(), quality_flags=())


def route_successor_taxi_out(*, flight_id: str, aircraft_id: str | None,
                             window_start: datetime, window_end: datetime,
                             events: tuple[OperationalEventRecord, ...],
                             proxy_event: OperationalEventRecord | None = None) -> RoutedTaxiOut:
    out_blocks = [event for event in events
                  if event.event_type == "TRAJECTORY_OUT_BLOCK_PROXY"
                  and _in_window(event, aircraft_id=aircraft_id,
                                 window_start=window_start, window_end=window_end)]
    takeoffs = [event for event in events
    ...
    if out_blocks and takeoffs:
        ...
        if takeoff.event_time <= out_block.event_time:
            return RoutedTaxiOut(flight_id=flight_id, taxi_out_minutes=None, source="NONE",
                support_state=SupportState.ABSTAIN,
                reason_code="TAXI_OUT_EVENT_ORDER_INVALID",
                parent_record_ids=(out_block.canonical_record_id, takeoff.canonical_record_id),
                quality_flags=())
        minutes = (takeoff.event_time - out_block.event_time).total_seconds() / 60.0
        support = SupportState.DEGRADED \
            if _DEGRADED_DETECTOR_FLAGS.intersection(out_block.quality_flags + takeoff.quality_flags) \
            else SupportState.SUPPORTED
        return RoutedTaxiOut(flight_id=flight_id, taxi_out_minutes=minutes,
            source="OUT_BLOCK_TAKEOFF_PAIR", support_state=support, reason_code=None,
            parent_record_ids=(out_block.canonical_record_id, takeoff.canonical_record_id),
    ...
    if proxy_event is not None and proxy_event.event_type == "ARCHIVE_FLIGHT_INTERVAL_PROXY":
        return RoutedTaxiOut(flight_id=flight_id, taxi_out_minutes=None, source="NONE",
            ...
            parent_record_ids=(proxy_event.canonical_record_id,), quality_flags=())
    return RoutedTaxiOut(flight_id=flight_id, taxi_out_minutes=None, source="NONE",
        support_state=SupportState.ABSTAIN,
        reason_code="TAXI_OUT_TRAJECTORY_PAIR_REQUIRED", parent_record_ids=(), quality_flags=())